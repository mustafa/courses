...
Understand the shift from regional to edge-first architecture
...
Understand Cloudflare's platform evolution timeline
...
-   **Everything is config files.** `wrangler.toml` is declarative. An agent can read your existing config, understand what you have, and add a KV namespace or D1 binding in seconds.
-   **Single deploy command.** `wrangler deploy` deploys to all 330+ cities simultaneously. No multi-region configs, no container registries, no Kubernetes yamls.
-   **Zero infrastructure management.** There are no EC2 instances to size, no VPCs to configure, no security groups. An agent can go from "create a URL shortener" to deployed production URL in under 2 minutes.
-   **The CLI is agent-friendly.** In 2026, Cloudflare ships the `cf` CLI alongside Wrangler — designed specifically for programmatic use by AI agents with structured JSON output and idempotent operations.
...
Understand why agents + Cloudflare is a powerful combination
...
Cold Start Latency Comparison (P95) | 2800ms |████████████████████████████ AWS Lambda (Node.js, cold) 1200ms |████████████████ AWS Lambda (Node.js, warm container) 800ms |██████████ Google Cloud Run (cold) 120ms |█ Vercel Edge Functions 50ms |▌ Cloudflare Workers (new isolate) 2ms |. Cloudflare Workers (cached isolate) └────────────────────────────────────────────────────── Zero config, 330+ PoPs, no warm-up required

**Why isolates are safe despite sharing a process**
...
Understand V8 isolates vs containers and cold start differences
...
**The egress fee is Cloudflare's biggest structural advantage**
...
Compare 2026 pricing across Cloudflare, AWS, Vercel, Netlify
...
Cloudflare Global Network (selected PoPs) Americas Europe/ME/Africa Asia Pacific ───────── ───────────────── ──────────── New York London Tokyo Los Angeles Frankfurt Singapore Miami Amsterdam Sydney Chicago Paris Mumbai Toronto Dublin Hong Kong São Paulo Stockholm Seoul Mexico City Warsaw Jakarta Seattle Madrid Taipei Atlanta Milan Osaka Dallas Zurich Bangkok Denver Lisbon Auckland Phoenix Nairobi Kuala Lumpur Minneapolis Lagos Manila ... 330+ total Every Worker request routes to the NEAREST city automatically. Latency from most users: under 20ms to first edge node.

Understand Cloudflare's network scale and Anycast routing
...
-   **Legacy runtime dependencies** — Workers use a subset of Web APIs, not Node.js APIs. If you need native modules (`bcrypt`, `sharp`), you'll need to find WASM alternatives.
-   **Large memory workloads** — Workers cap at 128MB. Parsing large CSV files or in-memory caching of big datasets needs a different approach.
...
Know the limitations and when to choose a different platform
...
1.  Request arrives at nearest Cloudflare PoP via Anycast
2.  The scheduler checks if an isolate for your Worker is already warm
3.  If warm (<1ms), the existing isolate handles the request
4.  If cold (<5ms), a new isolate is created by loading your compiled script
5.  Your `fetch` handler executes synchronously until it returns a Response
6.  The isolate may remain warm for the next request, or be garbage collected
...
Understand the V8 isolate execution model and request lifecycle
...
Write a basic Worker with routing and JSON responses
...
Set up TypeScript Workers with proper type bindings
...
Master the Wrangler CLI for development and deployment

### wrangler.toml — Full Configuration Reference
...
Configure wrangler.toml with all binding types
...
Implement middleware patterns for auth, CORS, and rate limiting
...
Build stateful Durable Objects for rate limiting and counters
...
Set up cron triggers for scheduled background tasks
...
Implement production error handling with structured logging
...
Understand WASM Workers for CPU-intensive tasks
...
-   **cf CLI** — a unified CLI alongside Wrangler, designed specifically for AI agent consumption. Features: structured JSON output (`--json` flag on all commands), idempotent operations, machine-readable error codes, and a `cf agent` subcommand for agent-specific workflows.
...
Learn 2026 Dynamic Workers and the cf CLI for agents

## Cloudflare Storage Stack

### The Storage Decision Tree

What kind of data are you storing? │ ├── Simple key-value pairs, session data, config, feature flags? │ └── → KV (Workers KV) │ └── Note: eventually consistent, reads cached at edge │ ├── Files, images, videos, user uploads, backups? │ └── → R2 (S3-compatible object storage) │ └── Note: zero egress, supports presigned URLs │ ├── Structured relational data, queries, relationships? │ └── → D1 (SQLite at the edge) │ └── Note: strong consistency, SQL queries, 10GB max │ ├── Already have Postgres/MySQL? Need to connect from Workers? │ └── → Hyperdrive (connection pooling proxy) │ └── Note: caches queries, pools connections globally │ ├── AI embeddings, similarity search, RAG? │ └── → Vectorize (vector database) │ └── Note: integrates with Workers AI natively │ └── Complex stateful data that needs strong consistency + real-time? └── → Durable Objects storage (transactional SQLite) └── Note: single writer, linearizable, WebSocket-friendly

Know when to use each Cloudflare storage primitive
...
Implement KV CRUD with TTL, metadata, and prefix listing
...
-- Run migration: wrangler d1 execute my-db --file=schema.sql
```
...
Build a D1 REST API with schema, migrations, and CRUD
...
```
// Worker using Hyperdrive with postgres.js
...
Connect an external Postgres database via Hyperdrive
...
Build semantic search with Vectorize and Workers AI embeddings
...
Understand what Cloudflare Pages is and how it differs from Vercel
...
Know framework support and build configurations for Pages
...
Build Pages Functions with file-based routing
...
-   Staging environments via protected `staging` branch
...
### Custom Domains, Redirects, and Headers
...
**VoidZero Acquisition — Vite Native Integration Coming**

In 2024, Cloudflare acquired VoidZero, the company behind Vite (the most popular frontend build tool). In 2026, this means: native Vite plugins for Workers and Pages, zero-config bundling for Workers, and a unified dev server that mirrors production exactly. If you're starting a new project in late 2026, expect `npm create cloudflare` to scaffold Vite-powered projects with full HMR support for Workers.

Configure custom domains, redirects, and security headers

### Full-Stack App Architecture Pattern

Full-Stack App on Cloudflare User Browser │ ▼ Cloudflare Edge (330+ PoPs) │ ├──► Pages — serves HTML/CSS/JS (CDN cached, instant) │ └── Astro/Next.js/SvelteKit frontend │ ├──► Pages Functions — /api/\* routes (serverless) │ ├── Auth middleware │ └── REST API handlers │ ├── D1 — user data, posts, orders (SQL) │ ├── KV — sessions, cache, feature flags │ └── R2 — file uploads, media │ └──► Workers — background services ├── Email Worker — transactional email ├── Queue consumer — async job processing └── Cron Worker — scheduled maintenance Total infra config: ~50 lines of wrangler.toml Deploy time: ~30 seconds globally

Design a full-stack architecture using Pages + Workers + storage
...
Understand Workers AI architecture and billing model
...
Know the 2026 Workers AI model catalog
...
Build streaming and non-streaming AI chat endpoints
...
Set up AI Gateway for caching, logging, and provider fallbacks
...
Build a complete RAG pipeline with Vectorize + Workers AI
...
Understand Workers AI pricing and the neuron model
...
**The Cloudflare + Claude Code Workflow**
...
Understand Cloudflare as the platform for internal tools
...
Build secure internal API endpoints with key authentication
...
Build a webhook receiver with signature validation
...
Build a Slack bot with slash commands and interactive components
...
- Include wrangler.toml bindings"
```

Use AI coding agent prompt templates for internal tools
...
Implement background processing with ctx.waitUntil()
...
Zero Trust Access Flow Employee Browser │ "https://admin.company.com/dashboard" ▼ Cloudflare Access │ "Who are you?" ├──► Not logged in → redirect to Google/Okta login │ ├──► Logged in, policy matches → pass through │ (email in allowed list, or group: "engineering") │ └──► Logged in, policy fails → 403 Access Denied (contractor, not in "admin" group) Your App Server (could be behind Cloudflare Tunnel) ↑ Never exposed to internet — only Cloudflare reaches it
...
Set up Cloudflare Access for Zero Trust internal app access
...
Configure WAF rules for SQL injection, country blocking, and threat scores

### DDoS Protection — Automatic and Unlimited
...
Understand Cloudflare's DDoS protection model
...
Set up Cloudflare Tunnel to securely expose local services
...
Implement Turnstile for bot-proof forms
...
Configure SSL/TLS modes and security headers
...
Implement edge caching with the Cache API and stale-while-revalidate
...
Build real-time WebSocket applications with Durable Objects
...
Implement async job processing with Cloudflare Queues
...
Process incoming email with Email Workers routing
...
Use Browser Run for headless browser automation at the edge
...
Use DO Facets for multi-tenant isolated SQLite storage

### NEW 2026: Artifacts — Git-Compatible Versioned Storage
...
Use Artifacts for versioned file storage in agent workflows
...
Implement zero-trust egress control with Outbound Workers
...
[[kv_namespaces]]
binding = "STATS"
id = "YOUR_STATS_NAMESPACE_ID"
```

```
// src/index.ts — complete URL shortener
export interface Env {
  LINKS: KVNamespace;
  STATS: KVNamespace;
...
Build the URL shortener with click tracking
...
Build the blog CMS with D1 schema, API, and pagination
...
Build the RAG chatbot with streaming and conversation history
...
Build the Slack standup bot with modals and channel posting
...
Build the admin dashboard with generic CRUD over D1
...
Build the cron monitoring service with alerts
...
Know the Lambda-to-Workers migration mapping
...
# or: wrangler pages deploy ./build-output --project-name=my-site
```

**Next.js Edge Runtime Caveat**

Not all Next.js features work on Cloudflare Pages — specifically, anything requiring full Node.js APIs (native modules, child\_process, fs in certain configurations). Use the `edge` runtime directive on pages/API routes: `export const runtime = 'edge'`. Pages using the default Node.js runtime won't work on Workers. Check `@cloudflare/next-on-pages` compatibility matrix before migrating.

Migrate from Vercel to Pages for Next.js, Astro, SvelteKit
...
Understand the free tier limits and plan accordingly
...
wrangler tail --search="TypeError"      # filter by error text
```

```
// Tail Worker — receives all log events from another Worker
...
Set up Tail Workers for production error alerting
...
# CF_ACCOUNT_ID: Your Cloudflare account ID (from dashboard)
```

Set up GitHub Actions CI/CD with staging and production environments

### Wrangler Environments — Staging and Production
...
# wrangler deploy --env staging
# wrangler deploy --env production
# wrangler dev --env staging  (local dev with staging bindings)
```

Configure multi-environment deployments with isolated resources

### Testing Workers with Vitest + Miniflare
...
Write unit and integration tests for Workers with Vitest + Miniflare
...
-   **Batch D1 queries** — use `env.DB.batch()` to combine multiple queries into one round trip. Reduces both latency and query count.
-   **Set appropriate KV TTLs** — expired KV values still count toward storage (until GC). Set `expirationTtl` on everything that should expire to keep namespace size down.
-   **Use conditional requests (ETag)** — return `304 Not Modified` for unchanged resources to avoid R2 reads and bandwidth costs.
-   **Logpush instead of console.log in prod** — `console.log` on every request burns CPU time. Use structured JSON and send logs via Logpush to your analytics platform.

Apply cost optimization strategies across Workers, KV, and D1
