...
-   **Anthropic — Claude Code hardening release (August 28):** Adds a `--restricted` mode that strips built-in command-execution tools and WebFetch, cross-session messaging improvements, and usage credits for Enterprise. Restricted mode matters: it's a first-class answer to running coding agents in environments where arbitrary execution is unacceptable.
