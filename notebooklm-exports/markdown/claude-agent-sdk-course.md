...
-   **Model Hardware Standard — Claude gets a contract with the physical world (August 28):** Anthropic released the Model Hardware Standard as a research preview: a software standard for Claude to drive robots, lab instruments, and manufacturing hardware. For SDK builders this is a new tool surface with the same shape as computer use — a standardized action interface plus safeguards — except the blast radius of a bad action is physical. If you build against it, treat every actuator command the way you treat a destructive shell command: gated, logged, and reversible where possible.
-   **Claude Code ships `--restricted` mode (August 28):** The new flag (or `CLAUDE_CODE_RESTRICTED=1`) removes the built-in tools that run commands or code, plus WebFetch. This is the first-party answer to embedding coding agents in environments where arbitrary execution is unacceptable — previously you had to deny-list tools yourself and hope you caught them all. The release also improves cross-session messaging (falling back to a private per-user /tmp directory) and adds usage credits for Enterprise.
-   **Claudeforce — Salesforce becomes a Claude surface (August 26):** The expanded partnership makes Salesforce data, workflows, business logic, and actions securely accessible from Claude; pilot now, open beta expected September. Watch the authorization model when it lands — enterprise systems of record exposing *actions* (not just data) to an agent is exactly where task-bounded permissions matter most.
-   **Claude Team plan for scientists (August 28):** 10,000 seats free or discounted for a year, alongside updated free K-12 content in Claude Academy. Anthropic keeps building distribution in research and education verticals while OpenAI expanded ChatGPT for Teachers to more districts the same week.
-   **Deadline: Sonnet 5 promo pricing ends August 31.** $2/$10 per Mtok reverts to $3/$15 on September 1. If your agents default to Sonnet 5 and you haven't reforecast, your costs rise ~50% tomorrow — this is the week to confirm model identity lives in config, not code.

### Week of August 23, 2026

-   **The agent stack is GA — delete the parts of your harness it replaces (August 19–20):** Computer use, the new *browser use* tool, the Skills API, and the Files API all left beta on the Claude Platform the same day. Three concrete consequences for SDK builders. First, computer use now takes several actions per turn rather than one per model call — early-access customers saw 20–40% fewer round trips, and one reported a claims workflow dropping from 32 to 13 minutes with ~30% lower cost per task and *no prompt changes*. If you built a loop that screenshots, decides, acts, and re-screenshots on every step, that loop is now the slow path. Second, browser use exposes the page's accessibility tree alongside the screenshot, so target elements semantically instead of by coordinate; pixel-targeting was always the brittlest part of these agents. Third, the Skills API means skills are uploadable and *versioned* server-side and run in Claude's sandbox — if you are shipping skill folders inside your own container image, you are hosting infrastructure you no longer need to. Files API adds automatic expiration, 5x rate limits, and 1TB per org; computer use is now HIPAA-eligible under a BAA. Existing beta integrations keep working during migration.
-   **Managed Agents — egress control and persistent state land together (August 19):** You can now set `allowed_domains` or `blocked_domains` on `web_search` and `web_fetch` in the `agent_toolset_20260401` configs array, with `max_content_tokens` on fetch and `user_location` on search. Separately, sessions in self-hosted sandboxes can attach memory stores, so agents persist state between sessions inside your own infrastructure. Read these two together: a domain allowlist is the cheapest available mitigation for indirect prompt injection, because it bounds which text can ever reach your agent's context. If your agent both fetches arbitrary web content *and* writes to a durable memory store, an injection is no longer a bad turn — it is a persistent one. Allowlist first, then persist.
-   **Python SDK v1.0 — a real migration, not a version bump (August 20):** The HTTP layer moves from `httpx` to httpx2 (API-compatible fork): build custom `http_client`, `Timeout`, and transport objects from `httpx2`, and call `httpx2.alias_httpx()` at startup if you use tracing or mocking libraries that patch `httpx`. Python 3.10+ is now the floor. Removed: the legacy Text Completions API, and `temperature`, `top_p`, `top_k` on Messages methods, plus the tool runner's client-side `compaction_control`. On the async client, `.with_raw_response` results now require `await response.parse()`, and `AnthropicBedrock` raises instead of silently defaulting to `us-east-1`. That last one will surface as a hard failure in exactly the deployments that were relying on the default without knowing it — check your Bedrock region config before upgrading. The v1 migration guide has before/after snippets for every change.
-   **Reforecast is now overdue — Sonnet 5 promo ends in eight days (August 31):** $2/$10 per Mtok reverts to $3/$15 on September 1. Meanwhile OpenAI cut GPT-5.6 Sol to $4/$20 (from $5/$30) promotionally through at least November 21. Two promo clocks running in opposite directions on the two models most agents default to is the clearest possible argument for the point this course keeps making: model identity belongs in configuration, and your cost model should be built on list price with promos treated as upside.
-   **Dual-use capability, gated by output shape (August 21):** Anthropic made Claude Mythos 5 available for Claude Security codebase scans on Enterprise plans — findings return with CWE category, confidence, severity, and a suggested patch requiring human approval — and is integrating Mythos into partner security tools where the end user never prompts the model directly. It also launched a $35M Defender Advantage Fund for open-source security and is extending the Cyber Verification Program toward Mythos-class access. The transferable design lesson: when your agent wraps a capability that is powerful in both directions, constrain what it can *emit* rather than trying to police what users ask for. A narrow output contract is a stronger boundary than a prompt-level policy, and it survives adversarial users in a way instructions do not.
-   **Claude Code 2.1.238–239 — the fixes worth knowing about:** A Bedrock streaming bug behind proxies that strip `Content-Type` was *silently doubling billed API calls* by re-running every turn non-streaming — check your Bedrock spend if you run behind a proxy. Remote MCP servers no longer stay dead after a transient 5xx on mid-session reconnect (this also affected SDK `setMcpServers()`). `headersHelper` in a project `.mcp.json` now requires the folder's trust dialog and runs without inherited credential env vars — a sensible tightening, but it will break helpers that quietly depended on ambient credentials. OpenTelemetry traces no longer fragment when a `PreToolUse` hook defers a tool call. Windows gained cross-session messaging (`SendMessage`/`ListAgents`), and `ListAgents` now reports a session its own addressable name.
-   **Ecosystem — MCP goes stateless, and a free curriculum appears:** An updated MCP roadmap landed August 22, building on the 2026-07-28 spec that removed transport-level session management entirely (the `initialize`/`initialized` handshake and `Mcp-Session-Id` header are gone), giving a stateless core that scales on ordinary HTTP load balancers — if you operate MCP servers, this is the migration to plan. Claude's connector directory now lists over 950 servers. Anthropic also opened **Claude Academy**, a free hub built from its own employee onboarding (4D AI Fluency Framework, ever-boarding, badges), notable for teaching durable mindsets — "verify in proportion to the stakes" — rather than model-specific prompt tricks. Finally, a 36-minute authentication outage on August 16 degraded claude.ai, Console, API, Claude Code, and Cowork simultaneously with no published root cause: a useful reminder that a single auth dependency is a single point of failure for every surface your agent runs on.
