...
[1What Is Cloudflare OS?](#mod1) [2Gadgets — A New Software Paradigm](#mod2) [3Gatekeepers — Secure Agent Access](#mod3) [4The Architecture](#mod4) [5Security Deep Dive](#mod5) [6Getting Started](#mod6) [7Coursera Analysis — Should We Adopt?](#mod7) [8Competitive Landscape](#mod8)

## Module 1: What Is Cloudflare OS?
...
-   **Repository:** [github.com/cloudflare/cloudflare-os](https://github.com/cloudflare/cloudflare-os) — note the name is `cloudflare-os`, not `os`. Apache 2.0.
-   **Homepage / hosted deploy flow:** [os.cloudflare.app](https://os.cloudflare.app)
-   **Deployment starter:** [github.com/cloudflare/cloudflare-os-starter](https://github.com/cloudflare/cloudflare-os-starter) — a wrapper repo for pinning a release and customizing it without patching upstream.
...
Ask agents to do tasks. The agent knows how *your* company operates — terminology, systems, conventions — because you loaded that context in. It is a [Code Mode](https://blog.cloudflare.com/code-mode/) agent: it performs tasks by writing and immediately executing code snippets, not by emitting tool-call JSON.
...
> **Warning:** Read this before you get excited
>
...
**Q: In what two senses does Cloudflare use the word "operating system"?**

-   A Linux distribution, and a container runtime
-   An OS for the company to be productive with AI safely, and an OS for managing AI workloads the way a kernel manages compute **(correct)**
-   A desktop environment, and a mobile OS
-   A CI system, and a package manager

*Explanation: Both senses are explicit in the README: an OS for the company to use AI safely so the security team can sleep, and an OS for AI workloads analogous to how a traditional OS manages compute workloads.*

**Q: What is the correct GitHub repository and license for Cloudflare OS?**

-   cloudflare/os, MIT
-   cloudflare/cloudflare-os, Apache 2.0 **(correct)**
-   cloudflare/workerd, BSD
-   cloudflare/workshop, AGPL

*Explanation: The repo is github.com/cloudflare/cloudflare-os under Apache 2.0. The deployment wrapper is a separate repo, cloudflare/cloudflare-os-starter.*

**Q: What does Cloudflare say the intended adoption pattern is?**

-   Companies subscribe to Cloudflare OS as a hosted SaaS product
-   Companies fork it and make it "Your Company OS" loaded with their own context and systems **(correct)**
-   Only Cloudflare runs it; the source is published for reference
-   It replaces your identity provider

*Explanation: The README states the idea is not that your company uses Cloudflare OS, but that you make it 'Your Company OS' — fork, customize, load your context.*
...
## Module 2: Gadgets — A New Software Paradigm
...
This is subtle and it is the reason the architecture choices in Module 4 exist. Every Gadget's client and server must communicate via [Cap'n Web RPC](https://github.com/cloudflare/capnweb). That requirement buys two things simultaneously:
...
**Q: When you create a slide deck in Cloudflare OS, what actually happens?**

-   A document is stored in a shared multi-tenant slides service
-   The system creates a private instance of the slide deck software, sandboxed just for you **(correct)**
-   A static file is rendered to R2
-   An MCP server proxies to Google Slides

*Explanation: The system creates a private instance of the software — a Gadget — running in a sandbox separate from everyone else's. That is what makes both cross-tenant leaks impossible and user modification safe.*

**Q: What does a Blueprint capture?**

-   Source code, binding requirements, and metadata — but not storage, chat history, or credentials **(correct)**
-   A full snapshot including the SQLite database and OAuth tokens
-   Only a screenshot and description
-   The running Durable Object, migrated to a new owner

*Explanation: Per docs/blueprints.md, a Blueprint captures the code snapshot, the shape of each required binding, and metadata. It deliberately excludes SQLite contents, chat/edit history, and any live connections or credentials.*

**Q: Why does Cloudflare OS require Gadget client-server communication to use Cap'n Web RPC?**

-   It is faster than HTTP
-   It is the only protocol Durable Objects support
-   Low boilerplate makes it easy for agents to write, and the resulting clean API is directly callable by an agent — so every Gadget gets an agent API for free **(correct)**
-   It provides end-to-end encryption

*Explanation: It is a win-win: low-boilerplate so agents write it easily, and it forces the server to expose an understandable API that the agent harness can invoke directly via Code Mode. No MCP server or custom agent loop needed.*

**Q: What is the main organizational risk introduced by the per-user-instance model?**

-   Higher compute cost than multi-tenant SaaS
-   Divergence — many forked copies producing inconsistent results, with no single source of truth **(correct)**
-   Users cannot collaborate in real time
-   Blueprints leak credentials between users

*Explanation: Per-user instances eliminate cross-tenant leaks and maintenance bottlenecks but create divergence. The mitigation is to put anything that must produce one canonical answer behind a Gatekeeper, not inside Gadget code.*

### 🃏 Flashcards

-   **Gadget** — A small application (client UI + server logic + persistent state) instantiated privately per user, running in its own sandbox. Not a document in a shared app — your own instance of the app.
-   **Blueprint** — A shareable snapshot of a Gadget's source code plus its binding requirements and metadata. Others stamp out independent copies with their own storage, bindings, and credentials. 128-bit hex ID; exportable as a .gadget file.
-   **Gadget vs Blueprint sharing** — Share the Gadget = collaborators join your live instance with shared real-time state. Share the Blueprint = they get their own independent copy of the code. Different security models entirely.
-   **Why per-user instances kill a whole bug class** — There is no shared multi-tenant datastore, so an app bug cannot leak one user's data to another. The sandbox — not the app's own access-control code — is what enforces isolation.
-   **Free multiplayer** — Every Gadget is backed by a Durable Object, so real-time collaboration is near-trivial — the coding agent implements it by default without being asked.
-   **Blueprint binding types** — gatekeeper (external resource connection), aiModel (a model binding), agentSpawner (agent-spawner config). The instantiating user supplies their own account and model for each.
-   **Divergence risk** — Forty forks of a revenue dashboard produce forty possible answers. Mitigation: canonical numbers live behind a Gatekeeper-governed API; Gadgets handle presentation and workflow only.

## Module 3: Gatekeepers — Secure Agent Access
...
> **Warning:** Documentation discipline that is easy to get wrong
>
...
| Google | gatekeeper-google | Yes | High — Docs, Drive, Gmail, Calendar |
...
**Q: What is the key innovation in how Gatekeepers handle human-in-the-loop approval?**

-   Approvals are pushed to Slack for faster response
-   The Gatekeeper simulates the action's outcome locally so the agent proceeds immediately, and the human approves in bulk later **(correct)**
-   Approvals are skipped for low-risk actions using an ML classifier
-   The agent waits, but with a longer timeout

*Explanation: Simulation is the breakthrough: the Gatekeeper tells the agent the action completed and returns simulated results on read-back, so the agent never blocks. The human approves or rejects later, in bulk or individually.*

**Q: Which ApprovalQueue call must a read-only operation make?**

-   submitAction()
-   applyAction()
-   authorizeObservation() **(correct)**
-   rejectAction()

*Explanation: Reads with no side effects call authorizeObservation() before returning data. Operations with externally-visible side effects call submitAction() and must not execute until applyAction().*

**Q: Why does the write-gatekeeper skill forbid mentioning submitAction/applyAction/approvals in the agent-facing types.d.ts JSDoc?**

-   To keep the file size small
-   Because correct simulation makes the approval machinery invisible, and leaking it into the agent's only documentation invites the agent to reason about and route around the control plane **(correct)**
-   Because JSDoc does not support those terms
-   Because approvals are deprecated

*Explanation: The types.d.ts is the agent's sole API documentation. Implementation and control-plane details belong in the .ts file or the PR — never in the agent-visible types.*

**Q: What are the three tiers of a Gatekeeper implementation?**

-   Client, Server, Database
-   Vendor (per service), User (per human's authenticated connection), Instance (per-resource, per-Gadget binding) **(correct)**
-   Read, Write, Admin
-   Proxy, Cache, Origin

*Explanation: GatekeeperVendor is the top-level service entry, GatekeeperUser holds one human's connected account and tokens, and Gatekeeper<Session> is the per-resource per-Gadget facet exposing the Session API.*

**Q: Which existing Gatekeeper package most reduces migration cost for an org that already runs MCP servers?**

-   gatekeeper-context
-   gatekeeper-mcp **(correct)**
-   gatekeeper-scheduler
-   gatekeeper-supabase

*Explanation: gatekeeper-mcp bridges existing MCP servers into the Gatekeeper model, so prior MCP investment is wrapped rather than rewritten — and inherits capability introduction, logging, and the approval queue.*

### 🃏 Flashcards

-   **Gatekeeper** — A service-specific Cloudflare Worker mediating all access between a Gadget/agent and an external service. Provides a Cap'n Web API, handles OAuth, enforces narrow resource access, logs every action, and gates side effects behind human approval.
-   **Simulation (async HITL)** — When an action needs approval, the Gatekeeper applies it locally and reports success, returning simulated results on read-back. The agent keeps working; the human batch-approves later. Solves the 'agent blocked while you got coffee' failure that drives people to auto-approve.
-   **authorizeObservation()** — Called before returning any externally-read data to the caller. Registers the read as an observation — which later constrains who the Gadget can be shared with (see observer verification).
-   **submitAction() / applyAction() / rejectAction()** — submitAction queues a side-effecting operation; the real effect only happens on applyAction. rejectAction discards it and unwinds any simulated state. Missing these on any method breaks the security model.
-   **Capability-shaped API design** — One interface per logical resource, where holding the object IS the permission — e.g. a ref to a specific Google Doc, not a method taking a docId. Narrowing authority means withholding objects or methods, not checking IDs inside every call.
-   **The two STOP gates** — The write-gatekeeper skill halts the agent twice: after designing types.d.ts (operator must approve the API before implementation) and before Phase 2. Cloudflare does not let an agent one-shot a security boundary.
-   **Phase 1 vs Phase 2** — Phase 1 = auth, API design, fine-grained granting (a working wrapper). Phase 2 = logging/approvals, caching, simulation, observer verification (an actual control point). Phase 2 is what makes it a Gatekeeper rather than an SDK.
-   **GATEKEEPER\_ prefix** — The workshop backend auto-discovers Gatekeeper vendors from service bindings whose names start with GATEKEEPER\_ in packages/workshop-backend/wrangler.jsonc. Registration is convention-based.

## Module 4: The Architecture
...
-   **Bring your own model.** Works with many major providers and self-hosted models. The heavy lifting is done by [Pi](https://pi.dev/) (`pi-agent-core`), which gives one API across providers.
...
**Q: In the Cloudflare OS / operating system analogy, what plays the role of device drivers?**

-   Durable Objects
-   Gatekeepers (packages/gatekeeper-\*) **(correct)**
-   Blueprints
-   The workshop-frontend shell

*Explanation: Drivers connect programs to external devices; Gatekeepers connect users and agents to external services. The kernel is workshop-backend, the shell is workshop-frontend, processes are gadgets, and executables are blueprints.*

**Q: What runtime primitive gives each Gadget its own V8 isolate and dedicated SQLite database?**

-   Containers
-   Durable Object Facets **(correct)**
-   Workers KV namespaces
-   R2 buckets

*Explanation: Every workspace is a Durable Object, and every Gadget runs in a Dynamic Worker Facet — a nested DO with its own isolate and SQLite. Isolates being cheap is what makes per-user app instances economically viable.*

**Q: What is "Code Mode" and why does it pair well with Cap'n Web?**

-   A read-only mode for viewing Gadget source
-   The agent writes and immediately executes code snippets instead of emitting per-step tool calls; Cap'n Web guarantees every Gadget and Gatekeeper exposes a clean typed API to write that code against **(correct)**
-   A syntax highlighting theme in Monaco
-   A billing tier for AI Gateway

*Explanation: Code Mode collapses multi-step tool use into executable snippets, which needs a good API to target. Cap'n Web forces exactly that API to exist — so the agent's tool surface and the app's client API are the same surface.*

**Q: Where does Blueprint code content actually live?**

-   In the Gadget's SQLite database
-   In Workers KV alongside the metadata
-   In R2, keyed by blueprintId/version, as a Yjs V2-encoded document **(correct)**
-   In the user's browser localStorage

*Explanation: Metadata propagates Gadget DO → User DO → Workers KV, but the code content is stored separately in the BLUEPRINT\_CONTENT R2 bucket keyed <blueprintId>/<version>, with old versions retained to avoid races during concurrent instantiation.*

### 🃏 Flashcards

-   **Durable Object Facet** — A nested Durable Object inside a parent DO, with its own V8 isolate and its own SQLite. Every Gadget runs in a Dynamic Worker Facet; Gatekeepers install facets into each workspace too.
-   **Cap'n Web** — Cloudflare's object-capability RPC system. You pass references, not identifiers — holding the reference IS the permission. Used for Gadget client↔server and for every Gatekeeper API.
-   **Code Mode** — The agent performs tasks by writing and immediately executing code snippets rather than emitting per-step tool-call JSON. Far fewer round trips and tokens for multi-step work.
-   **The kernel** — packages/workshop-backend. Connects users to Gadgets and Gatekeepers, sandboxes applications, and enforces access control — genuinely kernel-shaped responsibilities.
-   **The '???' row** — In Cloudflare's OS-analogy table, agents map to nothing in a traditional OS. Their argument: an agent is neither a user nor a service account — it's a third principal type, accountable to a human but with its own narrower capability-based permissions.
-   **AI Gateway's role here** — All inference routes through it, enabling model choice, cost tracking, and budget enforcement per user/team/workspace. It's the spend-control lever for an org rollout. Disabled by default in the deployment starter.
-   **Yjs in Cloudflare OS** — CRDT sync for Gadget code between clients and agents, plus history replay. It's why a human and an agent can edit the same Gadget concurrently without corruption. Blueprints store a stripped Yjs snapshot.
-   **Runtime features built for this product** — Dynamic Workers, Facets, and several other Workers Runtime features were added specifically to support Cloudflare OS. Explains both the elegance and the portability problem.

## Module 5: Security Deep Dive
...
**Q: How does Cloudflare OS differ from a typical MCP setup in how agents get access to services?**

-   It uses API keys instead of OAuth
-   Agents start with zero access and must be explicitly introduced to specific resources, rather than inheriting ambient access to all configured servers **(correct)**
-   It requires a VPN connection
-   Agents get read-only access by default, write access on request

*Explanation: With MCP, servers are configured up front and every session has broad ambient access to everything connected. In Cloudflare OS an agent holds only the capabilities it was introduced to for the job at hand — which structurally contains prompt injection.*

**Q: Alice's Gadget read a sensitive spreadsheet Bob cannot access. Alice shares the Gadget with Bob. What happens?**

-   Bob sees the Gadget; the spreadsheet data is redacted automatically
-   Bob must supply his own connected account, and the Google Gatekeeper verifies he could directly read everything the Gadget has read — he fails, so he is denied access entirely **(correct)**
-   Bob inherits Alice's credentials for the duration of the session
-   Nothing is enforced; this is a known gap

*Explanation: Observer verification: Bob supplies his own connected accounts, each Gatekeeper independently verifies his access against the Gadget's observation history inside its own trust domain, and v1 is all-or-nothing — he is denied, not partially served.*

**Q: Why does the restricted "use" session class implement the full Overseer interface?**

-   For backwards compatibility with older clients
-   So that any newly added interface method fails to compile until a developer consciously decides whether "use" callers may invoke it — the compiler enforces default-deny **(correct)**
-   To reduce bundle size
-   Because Cap'n Web requires it

*Explanation: It's a default-deny-by-construction trick: because the class implements Overseer, adding a method to the interface breaks the build until someone explicitly classifies it. The type system enforces the security review.*

**Q: Which threat does the Cloudflare OS security model NOT handle?**

-   A Gadget exfiltrating data to an attacker-controlled endpoint
-   A cross-user data leak from an app bug
-   A buggy or malicious custom Gatekeeper that skips submitAction() **(correct)**
-   Sharing a Gadget with someone who lacks access to the data it read

*Explanation: Gatekeepers are the trusted computing base. One that skips authorizeObservation() or submitAction() silently disables observer verification or approvals on that path. Custom Gatekeepers require the same review bar as authentication code.*

**Q: How are share links stored server-side?**

-   Encrypted with a rotating key
-   Only as an HMAC-SHA-256 hash of a random 128-bit key, so a database leak cannot reveal valid links **(correct)**
-   In plaintext, scoped to the Gadget
-   As JWTs signed by the workspace DO

*Explanation: The server stores only the HMAC-SHA-256 hash with a domain-separation constant. It cannot reconstruct share links from stored data, and 'copying' a link mints a new key rather than revealing the original.*
...
A guided flow at [os.cloudflare.app/deploy](https://os.cloudflare.app/deploy) deploys the upstream release into your own Cloudflare account. It builds nothing locally, configures sign-in and admin emails for you, and leaves the whole `/admin` surface intact: site name, logo, accent color, announcements, agent instructions, featured Blueprints, and which connectors your users can reach. Built-in Gatekeepers are yours to connect with your own OAuth credentials.
...
[cloudflare/cloudflare-os-starter](https://github.com/cloudflare/cloudflare-os-starter) wraps a *pinned* upstream release with deployment controls, without modifying upstream source. This is how a company should run it.
...
**Q: What is the fastest way to see Cloudflare OS working?**

-   Deploy to a Cloudflare account with the starter repo
-   Install pnpm and run \`pnpm run-local\`, then visit localhost:8787 **(correct)**
-   Sign up for a hosted trial
-   Build workerd from source

*Explanation: \`pnpm run-local\` runs the whole stack locally on wrangler and workerd, storing data in a .wrangler subdirectory. Not for production, but the right first move.*

**Q: What is the actual status of self-hosting on workerd?**

-   Fully supported and documented
-   Possible in principle — local dev already uses workerd — but the deployment documentation and tooling are marked COMING SOON **(correct)**
-   Not possible; Cloudflare OS requires Cloudflare's network
-   Available only under a commercial license

*Explanation: The README says Cloudflare OS can run entirely on workerd, but that docs and tooling for deploying on your own servers are still being worked on. Treat 'we can always self-host' as a roadmap promise, not a current capability.*

**Q: What is Cloudflare's stated policy on external code contributions?**

-   All PRs welcome under a CLA
-   They are not seeking outside contributions — AI made writing code easy, so external PRs donate the easy part and add review burden; only small trivially-verified fixes are accepted **(correct)**
-   Contributions accepted only from Cloudflare customers
-   The project accepts contributions but requires paid support

*Explanation: This matters strategically: any customization you cannot express as a custom Gatekeeper or a deployment.jsonc setting becomes a fork you maintain forever against a fast-moving upstream that will not merge it back.*

**Q: Which Cloudflare products must be available on the account for a starter-repo deployment?**

-   Workers and R2 only
-   Workers, KV, R2, Browser Rendering, and Dynamic Worker Loaders (AI products optional) **(correct)**
-   Workers, D1, and Pages
-   Only Workers AI and AI Gateway

*Explanation: Workers, KV, R2, Browser Rendering, and Dynamic Worker Loaders are required. AI is optional and disabled by default — you explicitly opt into Workers AI and AI Gateway.*

## Module 7: Coursera Analysis — Should We Adopt This?
...
**Q: What does a DNS and header check of coursera.org show about its edge provider?**

-   Cloudflare, so adopting Cloudflare OS is nearly free
-   AWS CloudFront fronting an envoy origin — so a Cloudflare account, contract, and security review become real line items **(correct)**
-   Fastly
-   No CDN

*Explanation: www.coursera.org resolves via www-cloudfront-alias.coursera.org to AWS ranges, with via/x-amz-cf-pop/x-amz-cf-id CloudFront headers. Cloudflare OS need not sit in the production path, but 'we're already on Cloudflare' cannot support the business case.*

**Q: Which use case should the pilot start with?**

-   Learner analytics — highest transformative value
-   Support ticket resolution with learner context
-   Documents, decks, code and product work using built-in Gatekeepers plus company context **(correct)**
-   Enterprise customer usage reporting

*Explanation: Effort and risk correlate almost perfectly, and both are lowest for document/code/product work. Anything needing a custom Gatekeeper over learner data is phase two, gated on erasure and retention answers.*

**Q: What is the most important design rule for a Coursera learner-analytics Gatekeeper?**

-   Cache aggressively to reduce warehouse cost
-   Design the API so row-level learner data cannot be expressed at all — only aggregates above a minimum cohort size **(correct)**
-   Require two approvals for every query
-   Log all queries to the data warehouse

*Explanation: Capability-based security works by withholding objects, not filtering results. If the API can return a single learner's record, every downstream control becomes a mitigation rather than a guarantee.*

**Q: Which risk should rank first for Coursera specifically?**

-   Cloudflare deprecating the project
-   Learner data erasure — per-user instances mean data could land in thousands of separate SQLite databases with no way to enumerate them **(correct)**
-   AI inference cost
-   The UI being rough

*Explanation: Per-user instances are excellent for confidentiality and awkward for erasure: you trade one queryable datastore for thousands of small ones. Get a written answer before any pilot touches production learner data.*

**Q: What makes the bounded pilot an easy call despite the platform being early access?**

-   Cloudflare offers free support
-   The asymmetry: downside is roughly one engineer-quarter, upside is a working governed self-owned agent platform plus organizational expertise in capability-based security **(correct)**
-   It replaces all existing tooling immediately
-   There are no competing options

*Explanation: Bounded downside against significant option value, plus the Gatekeeper pattern being worth learning by building one even if the platform is never expanded.*
...
> **Warning:** What would have to be true for this to win the category
>
...
**Q: What most fundamentally separates the Gadgets model from Retool and similar internal-tool platforms?**

-   Better AI models
-   One app instance per user with platform-enforced isolation, versus one shared multi-tenant app with permissions enforced inside the app **(correct)**
-   Lower cost
-   More connectors

*Explanation: Per-user instances make end-user modification safe and eliminate cross-tenant leaks structurally. It is an architectural commitment, not a feature competitors can add incrementally.*

**Q: Which capability does Cloudflare OS have that none of the listed alternatives offer?**

-   Real-time multiplayer collaboration
-   Observer verification — the source system adjudicates, per collaborator, whether they could have read everything a shared artifact was built from **(correct)**
-   Support for multiple LLM providers
-   OAuth-based connectors

*Explanation: Others answer the shared-output leak question with 'nothing' or 'a policy.' Cloudflare OS enforces it at the platform layer, per Gatekeeper, against a recorded observation history.*

**Q: Which development would most change the adoption recommendation?**

-   More prebuilt Gatekeepers
-   workerd self-hosting shipping as a genuinely supported path **(correct)**
-   A nicer UI
-   Cheaper Workers pricing

*Explanation: Self-hosting converts the largest objection — vendor dependence and the unfinished escape hatch — into the platform's biggest advantage over closed alternatives. It is the item to watch.*

### 🃏 Flashcards

-   **vs Retool** — Retool: one shared app, permissions inside the app, end users can't modify, someone owns it forever. Cloudflare OS: per-user instance, platform-enforced isolation, users modify their own copy via AI, Blueprints remove the maintenance owner.
-   **vs custom ChatGPT/Claude deployments** — They produce text with ambient connector access and no approval queue or observation tracking. Cloudflare OS produces running software with per-task capability introduction and governed writes. Much lower friction on their side, much stronger governance on this side.
-   **vs Copilot Studio** — Deep Microsoft integration and enterprise identity, but closed, estate-locked, tenant-wide agent permissions, and you don't own what you build. Cloudflare OS is Apache 2.0 in your own account.
-   **vs plain MCP + coding agent** — Free and flexible but engineer-only, ambient access, synchronous approvals that get disabled, and no sharing or observation model. gatekeeper-mcp is the bridge: wrap existing MCP servers rather than rewriting them.
-   **The two separating axes** — (1) Who runs the instance — one shared vs one per user. (2) How the agent gets authority — ambient preconfigured connectors vs zero-by-default with per-task introduction.
-   **Where Cloudflare OS loses** — When you need something today; when a deep Microsoft/Google estate makes single-vendor integration decisive; when the use case is retrieval not action; when your users are engineers already happy with an IDE agent.

## Module ⚑: Your Action List & Sources
...
-   [github.com/cloudflare/cloudflare-os](https://github.com/cloudflare/cloudflare-os) — README, `docs/blueprints.md`, `docs/sharing.md`, `docs/observers.md`, and `.agents/skills/write-gatekeeper/SKILL.md`. Apache 2.0.
-   [github.com/cloudflare/cloudflare-os-starter](https://github.com/cloudflare/cloudflare-os-starter) — deployment controls, `deployment.jsonc`, customization and observability guides.
-   [blog.cloudflare.com/cloudflare-os](https://blog.cloudflare.com/cloudflare-os/) — the launch post.
-   [Cloudflare press release](https://www.cloudflare.com/press/press-releases/2026/cloudflare-os-is-the-first-ai-workspace-built-around-how-companies-actually-work/), August 5, 2026.
-   [os.cloudflare.app](https://os.cloudflare.app) — product site and hosted deploy flow.
-   Supporting Cloudflare posts: [Code Mode](https://blog.cloudflare.com/code-mode/), [Dynamic Workers](https://blog.cloudflare.com/dynamic-workers/), [Durable Object Facets](https://blog.cloudflare.com/durable-object-facets-dynamic-workers/), and [Cap'n Web](https://github.com/cloudflare/capnweb).
-   Coverage: [SiliconANGLE](https://siliconangle.com/2026/08/05/cloudflare-launches-cloudflare-os-open-source-ai-agentic-workspace-enterprise/), [Phoronix](https://www.phoronix.com/news/Cloudflare-OS), [Decrypt](https://decrypt.co/374997/cloudflare-os-inside-open-source-ai-agent-platform), [SD Times](https://sdtimes.com/cloud-integration/cloudflare-announces-open-source-ai-workspace-for-every-employee/).
-   Coursera edge-provider finding: DNS resolution and HTTP response headers for `coursera.org`, checked August 5, 2026.
