...
**Step 1 — Service Delivery & Documentation:** The provider renders care and documents it using standardized codes. A physician visit gets coded with an ICD-10 diagnosis code (what's wrong) and CPT procedure codes (what was done). The front desk collects a copay if applicable, which becomes relevant during adjudication because it counts toward accumulators.
...
**Step 7 — Pricing & Repricing:** For in-network providers, payment is determined by the contracted rate. The repricing engine takes the provider's billed charge and replaces it with the negotiated rate. For out-of-network providers, different logic applies — usual and customary (U&C) rates, reference-based pricing, or balance billing rules depending on plan design. This is where companies like MultiPlan operate, doing out-of-network repricing.
...
EDI TRANSACTION FLOW — CLAIMS SUBMISSION TO PAYMENT
  ...
  PROVIDER                 CLEARINGHOUSE              PAYER/TPA
  SYSTEM                   (Change/Availity)           (Gravie/Cigna)
  ─────────                ────────────────            ──────────────
     │                           │                          │
     │──── 837 (claim) ─────────►│                          │
     │                           │──── validate/route ──────►│
     ...
     │                           │                          │
     │◄─── 835 (remittance) ─────│◄─── 835 (payment) ───────│
     │                           │                          │
     │                                                      │
  ───────────────────────────────────────────────────────────
  SEPARATE FLOW: Eligibility
     │──── 270 (inquiry) ────────────────────────────────────►│
     │◄─── 271 (response) ───────────────────────────────────│
  ───────────────────────────────────────────────────────────
  SEPARATE FLOW: Enrollment
  EMPLOYER HR ──── 834 (enrollment) ─────────────────────────►│
                           │                          │
```
...
| TPA (Third Party Administrator) | Employer bears risk (self-funded) | DOL / ERISA (federal); limited state oversight | Employer funds claims account; TPA draws against it per claim | Employer owns data; TPA is custodian | Gravie, Collective Health, Meritain, HealthSCOPE |
...
**8\. Payment and Disbursement:** Generates payments to providers (EFT, check, or virtual card) and produces 835 remittance advice. For self-funded plans, this module draws against the employer's claims funding account — requiring integration with the employer's bank account or a custodial account maintained by the TPA. Payment timing is contractually governed (Clean Claims Act requirements in most states mandate payment within 30-45 days for fully insured plans; ERISA governs self-funded). The member-facing component handles reimbursement for out-of-pocket claims and EOB generation.
...
CORE DATA MODEL — HEALTH CLAIMS SYSTEM
  ...
  EMPLOYER ──────────────── PLAN ──────────────── BENEFIT_CONFIG
  (group_id,               (plan_id,              (benefit_id,
   name,                    group_id,              plan_id,
   funding_type)            plan_year,             service_category,
        │                   plan_type)             copay, coinsurance,
        │                        │                 deductible_apply,
        │                        │                 oop_apply,
        │                        │                 coverage_pct)
        │                        │
  ...
  (enrollment_id,          (member_id,
   ...
   plan_id,                 group_id,
   effective_date,          name, dob,
   term_date,               relationship)
   premium_tier)                 │
                                 │
                     ┌───────────┼───────────┐
                     │           │           │
                  CLAIM       ACCUM      ACCUMULATOR
                 (claim_id,  (accum_id,  _BALANCE
                  member_id,  member_id, (balance_id,
                  group_id,   plan_id,    accum_id,
                  icn,        accum_type, amount_met,
                  dos,        plan_year)  last_updated)
                  claim_type,
                  status,
                  total_billed,
                  total_allowed,
          ...
          (npi,              _CONTRACT
           name,             (contract_id,
           tax_id,            npi,
           specialty,         network_id,
           entity_type)       eff_date,
                │             term_date,
                │             rate_type,
           NETWORK            rate_value)
          (network_id,
...
The adjudication engine is the intellectual center of a claims system. It is fundamentally a decision tree executed against a rich context object (the claim enriched with member, plan, provider, and accumulator data). The rules execute in a defined sequence because order matters — a coverage determination failure preempts the need for pricing logic.
...
ADJUDICATION FLOW — CLAIM STATE TRANSITIONS
  ...
  │  • NPI valid and not excluded?      │
  └─────────────────────────────────────┘
      │ PASS                   │ FAIL
      ▼                        ▼
  [ACCEPTED]              [REJECTED]
      │                   (277CA w/ rejection code)
      ▼
  ┌─────────────────────────────────────┐
  │  ELIGIBILITY CHECK                  │
  │  • Member enrolled on DOS?          │
  │  • Plan active on DOS?              │
  │  • Dependent relationship valid?    │
  └─────────────────────────────────────┘
      │ PASS                   │ FAIL
      ▼                        ▼
  [ELIGIBLE]               [DENIED]
  ...
  │  • Prior auth required + obtained?  │
  │  • Benefit limit not exhausted?     │
  │  • Coordination of Benefits needed? │
  └─────────────────────────────────────┘
      │ COVERED                │ NOT COVERED / NEEDS REVIEW
      ▼                        ▼
  [COVERED]               [DENIED or PENDED]
      │
      ▼
  ┌─────────────────────────────────────┐
  │  PRICING / REPRICING                │
  │  • In-network? → contracted rate    │
  │  • Out-of-network? → UCR / RBP      │
  │  • Apply modifiers, DRG, per diem?  │
  └─────────────────────────────────────┘
      │
      ▼
  ┌─────────────────────────────────────┐
  ...
  │  • Apply copay (per benefit config) │
  │  • Apply coinsurance                │
  │  • Apply OOP max (if met → 100%)    │
  │  • Update accumulators              │
  └─────────────────────────────────────┘
      │
      ▼
  ...
  │  • Clinical edit (CCI edits)        │
  │  • Unbundling / upcoding flags      │
  └─────────────────────────────────────┘
      │ PASS                   │ FLAG
      ▼                        ▼
  ...
  └──────────┘
      │
      ▼
  Payment Generation → 835 Remittance → EOB
```
...
Lasering is the stop-loss carrier's mechanism for excluding or surcharging known high-cost individuals from coverage. During underwriting, the stop-loss carrier reviews the group's medical history. A member with a $400,000 chronic condition may be "lasered" — their specific attachment point is set at $800,000 instead of $100,000, effectively removing stop-loss protection for that member. For the TPA, lasers appear as member-level exceptions in the stop-loss configuration that must be tracked and correctly applied when generating stop-loss reports. Failing to track lasers means the employer incorrectly believes they have stop-loss protection they do not have — a fiduciary failure and a financial time bomb.
...
EMPLOYER
  │
  ├─── Monthly PMPM funding ──────────────────────────────────────────┐
  │                                                                   │
  ├─── Stop-loss premium ──────────► STOP-LOSS CARRIER               │
  │         (monthly)                     │                           │
  │                                       │ Reimbursement when        │
  │                                       │ claims > attachment       │
  │                                       ▼                           ▼
  │                               STOP-LOSS              CLAIMS TRUST ACCOUNT
  │                               REIMBURSEMENT  ────►   (employer-funded)
  │                                                           │
  │                               GRAVIE (TPA)                │
  │                                   │                       │
  ├─── Admin fees (PMPM) ─────────────┘                       │
  │         (monthly)                                         │
  │                                                           │
  │         MEMBER submits / PROVIDER submits claim           │
  │                     │                                     │
  │                     ▼                                     │
  │         CLEARINGHOUSE (Change Healthcare/Availity)        │
  │                     │                                     │
  │                     ▼                                     │
  │         GRAVIE ADJUDICATION ENGINE                        │
  │              │          │                                 │
  │              │          └─► EDI 835 EOB to member/prov    │
  │              │                                            │
  │              └─► APPROVED CLAIM ──────────────────────────┤
  │                  (remittance advice)                      │
  │                                                           │
  │                  Claims Trust Account ────► PAYMENT ─────►PROVIDER
  │                                              (EFT/check)
  │
  └─── Annual reporting:
       - Stop-loss claims reports → Stop-loss carrier
       - 5500 filing → DOL (ERISA compliance)
       - ACA reporting (1094/1095) → IRS/Employees

NETWORK LEASE (Cigna ORCA / Aetna Signature / etc.)
  │
  └─► Contracted fee schedules used by Gravie adjudication engine
      (Gravie does not contract directly with all providers)
```

### The TPA Tech Stack: Gravie vs. a Full Carrier

The technology requirements for a TPA and a full carrier overlap significantly but diverge in critical areas. A carrier like UnitedHealth Group runs a vertically integrated stack: actuarial pricing engines, underwriting systems, group enrollment platforms, fully proprietary adjudication (Optum's ClaimLogiq, internally built systems), pharmacy benefit management (OptumRx), care management platforms, utilization management engines (InterQual integrated), member portals, provider portals, and a massive analytics infrastructure for risk adjustment and population health. The carrier owns the network contracts, maintains the fee schedule database for every CPT × provider × geography combination, and bears the financial risk — so every technology component ties back to managing that risk.
...
When Gravie makes discretionary claims decisions — approving or denying a claim based on its interpretation of the plan document — it may be acting as a functional fiduciary. This has legal consequences: fiduciaries can be personally liable for breaches, and the DOL has enforcement authority. In practice, most TPAs carefully draft administrative services agreements to limit discretionary authority and retain plan sponsor decision-making on appeals, specifically to avoid fiduciary status. But the operational reality is that the adjudication engine is making quasi-discretionary decisions constantly — applying clinical editing rules, applying network access logic, applying benefit matrix mappings — and errors in those decisions create fiduciary exposure.
...
└─────────────────────────────────────────────────────────────────────┘
               ...
               ▼                             ▼
┌──────────────────────────┐   ┌─────────────────────────────────────┐
│  MODERN/CLOUD-NATIVE     │   │  TPA-SPECIFIC PLATFORMS             │
│  ADJUDICATION            │   │  Alegeus (HRA/FSA/HSA admin)        │
│  Centauri Health (Clover) │   │  Benefitfocus (enrollment+admin)    │
│  Healthfirst (in-house)   │   │  BenefitMall (payroll-adjacent TPA) │
│  Beam Dental (vertical)   │   │  WEX Health (CDH accounts)          │
│  Architecture: API-first, │   │  Businessolver (benefits admin)     │
│  event-driven, cloud      │   │  Target: Employers, brokers, TPAs   │
└──────────────────────────┘   └─────────────────────────────────────┘
               │                             │
               └──────────────┬──────────────┘
                              ▼
               ...
               │  Function: Drug adjudication,│
               │  formulary, rebate mgmt      │
               └──────────────────────────────┘
                              │
                              ▼
               ┌──────────────────────────────┐
...
Gravie Comfort's $0 copay structure with bundled ancillary benefits is a genuine build justification case. Legacy adjudication systems like Facets have benefit configuration models built around traditional deductible/coinsurance/copay structures. Configuring a plan where a specific subset of CPT codes has $0 cost-sharing, where the benefit structure bundling dental and vision into a medical plan is seamless, and where the member-facing experience shows this cleanly requires either building on a flexible platform or fighting against an inflexible one. When "vendor customization" costs more in implementation fees and maintenance complexity than building the capability, build wins on total cost of ownership.
...
The technical requirements for a successful hybrid architecture are: (1) the core adjudication vendor must provide a real-time or near-real-time API for claim status, accumulator balances, and EOB data — not just nightly batch extracts, (2) the vendor's data model must be mappable to an internal canonical data model that the custom-built layers consume, (3) the vendor must support webhooks or event streaming so that claim events (submitted, pended, approved, denied, appealed) propagate immediately to downstream systems, and (4) the benefit configuration layer must be accessible enough that Gravie's operations team can configure new plan designs without filing a vendor change request. Most legacy systems fail on points 3 and 4. Modern API-first vendors (HealthEdge's newer cloud platform, purpose-built systems like those from Centivo or Imagine360) are better positioned here.
...
Parallel running — operating both old and new systems simultaneously and comparing outputs before cutting over — is the standard risk mitigation approach. It requires running both systems on production claim volumes, reconciling every discrepancy, and building confidence that the new system produces identical outputs to the old system for all claim types before any cutover. Parallel running at meaningful scale typically requires 3-6 months and doubles operational complexity during that window. The engineering cost is not just the new system — it is the reconciliation tooling, the ops team supporting two systems, and the incident response capacity to handle discrepancies. Budget 20-30% additional engineering capacity during a parallel run period.
...
| Vendor Lock-in Risk | 8% | 5 — no lock-in, full portability | 2 — migration costs are prohibitive; vendor has leverage | Facets/QNXT migrations are notoriously difficult; factor in whether the vendor is a consolidation target (Change Healthcare/Optum example) |
...
| Benefit Configuration Engine (Comfort-style plans) | Build or buy with deep API access | $0 copay + bundled ancillary = non-standard benefit matrix that most vendors cannot represent cleanly; this is where plan design IP lives | Custom-built on top of vendor adjudication APIs |
...
| Stop-Loss Tracking | Build (thin layer on adjudication data) | Stop-loss configuration is per-employer with laser exceptions; vendor systems support it poorly; the financial stakes of errors are too high to accept vendor black-box behavior | Custom accumulation service consuming adjudication events |
...
| Law / Framework | Who It Applies To | Key Claims System Obligation | Primary Enforcement Body | Penalty Range |
| --- | --- | --- | --- | --- |
| HIPAA Privacy Rule | Covered entities, BAs | Minimum necessary access, BAAs, PHI handling procedures | HHS OCR | $100–$50,000 per violation, up to $1.9M/year per category |
| HIPAA Security Rule | Covered entities, BAs | Technical, physical, admin safeguards for ePHI | HHS OCR | Same as above; criminal up to $250K + 10 years |
| HIPAA Breach Notification | Covered entities, BAs | 60-day notice, media notice for 500+ in a state | HHS OCR | $100–$50,000 per violation |
| ERISA | Self-funded employer plans, TPAs as fiduciaries | Fiduciary-grade denial documentation, SPD accuracy, Form 5500 data | DOL, IRS | Civil liability; $110/day for SPD violations |
| ACA (EHB, preventive, OOP caps) | Non-grandfathered group and individual plans | Correct preventive care $0 cost-sharing logic, OOP accumulator accuracy | HHS, DOL, IRS (trifecta) | $100/day/individual for non-compliance |
| No Surprises Act | Group health plans, carriers, providers | NSA claim identification, QPA calculation, MRF generation | HHS, DOL, Treasury, CMS | $10,000 per violation for providers; plan corrective action |
| State Prompt Pay Laws | Carriers, some TPAs (state-specific) | Pay clean claims within 30–45 days | State DOI | Interest + fines; license sanctions |
| CMS / Medicare Advantage | MA plans, MA-PD plans | PDE reporting, RADV readiness, Star Rating quality measures | CMS | Contract termination, large-scale payment recoupment |
| SOC 2 Type II | SaaS/technology service providers handling client data | Annual third-party audit of controls; required for enterprise sales | AICPA standards; customer contractual | Loss of enterprise contracts; reputational |
...
```
┌─────────────────────────────────────────────────────────────────┐
│                     REGULATORY SURFACE LAYER                     │
│  HIPAA │ ERISA │ ACA/NSA │ State Regs │ CMS/MA │ SOC 2 Type II  │
└────────────────────────────┬────────────────────────────────────┘
                             │  Requirements flow down
┌────────────────────────────▼────────────────────────────────────┐
│                    POLICY & CONTROL LAYER                        │
│  • BAA Registry & Tracking                                       │
│  • Access Control Policy (RBAC / ABAC with PHI tagging)         │
│  • Incident Response Runbooks (60-day HIPAA clock)              │
│  • Claims Rules Version Control (effective-dated configs)        │
│  • MRF Generation Pipeline (monthly refresh)                     │
│  • Prompt Pay SLA Monitoring by State                            │
└────────────────────────────┬────────────────────────────────────┘
                             │  Controls enforced by
┌────────────────────────────▼────────────────────────────────────┐
│                  APPLICATION CONTROL LAYER                       │
│  • Adjudication Engine (rules versioned, audited)               │
│  • Cost-Sharing Accumulator (OOP/deductible enforcement)        │
│  • NSA Logic (surprise billing detection, QPA calculation)       │
│  • Preventive Care Override (zero cost-share enforcement)        │
│  • Denial Reason Code Engine (fiduciary-grade explanations)      │
└────────────────────────────┬────────────────────────────────────┘
                             │  All decisions flow into
┌────────────────────────────▼────────────────────────────────────┐
│                   IMMUTABLE AUDIT LAYER                          │
│  • Append-only claim event log (hash-chained)                    │
│  • Rule evaluation trace per claim                               │
│  • User action log (overrides, appeals, voids)                   │
│  • Configuration change log (who changed what rule, when)        │
│  • Access log (who viewed which PHI records)                      │
└────────────────────────────┬────────────────────────────────────┘
                             │  Retained in
┌────────────────────────────▼────────────────────────────────────┐
│                  DATA PERSISTENCE LAYER                          │
│  • Encrypted at rest (AES-256), TLS 1.2+ in transit             │
...
CLAIMS SYSTEM INTEGRATION MAP
         ...
         ┌─────────────┐        ┌──────────────────────────────────────┐
         │   PROVIDERS  │──837──▶│         CLEARINGHOUSE LAYER          │
         │  (Hospitals, │◀──835──│   Change Healthcare │ Availity       │
         │   Physician  │        └─────────────┬────────────────────────┘
         │   Groups)    │                      │  Normalized 837/835
         └─────────────┘                      ▼
                                   ┌──────────────────────┐
    ┌──────────────┐    834        │                      │    834
    ...
    ┌──────────────┐              │  ENGINE           │         │   claims feed)│
    │    PBM       │◀────────────▶│  + Rules Engine   │         └───────────────┘
    │  CVS/ESI/    │  formulary   │  + Accumulator    │
    │  OptumRx/    │  + real-time │  + Network Logic  │
    │  Capital Rx  │  adj data    └────────┬──────────┘
    └──────────────┘                       │
                                  ┌────────▼───────────────────────────────────┐
                                  │              OUTPUT LAYER                   │
                       ┌──────────┴───────────────────────────────────────┐    │
                       │  Banking / ACH  │  Provider Payment, ACH         │    │
                       │  (NACHA files)  │  Member Reimbursement          │    │
                       └─────────────────────────────────────────────────┘    │
                       ┌──────────────────────────────────────────────────┐   │
                       │  Member Portal / App  │  EOBs, Claims Status,    │   │
                       │                        │  Cost-share Accumulators │   │
                       └──────────────────────────────────────────────────┘   │
                       ┌──────────────────────────────────────────────────┐   │
                       │  Employer Dashboard   │  Utilization Reports,    │   │
                       │                       │  Cost Trends, Population │   │
                       └──────────────────────────────────────────────────┘   │
                       ┌──────────────────────────────────────────────────┐   │
                       │  Data Warehouse       │  Snowflake / Databricks  │   │
                       │  (Analytics Layer)    │  / Redshift + BI tools   │   │
                       └──────────────────────────────────────────────────┘   │
                                  └────────────────────────────────────────────┘
```
...
The health insurance industry was built on EDI batch processing — X12 transactions submitted in bulk files, processed overnight, and reconciled the next business day. This model made sense when network bandwidth was expensive, computing was centralized, and providers sent claims in paper batches that were then keypunched. It makes much less sense in 2024, when members expect real-time eligibility verification, providers want immediate claim status, and employer clients want same-day utilization data. The tension between EDI batch and API-first architectures defines the modernization challenge for most claims systems.
...
EVENT-DRIVEN CLAIMS PROCESSING ARCHITECTURE
  ...
  ┌─────────────┐    ┌──────────────────────────────────────────────────┐
  │ CLEARINGHOUSE│───▶│  INGESTION SERVICE                               │
  │  (837 files) │    │  • Format validation (X12 5010)                  │
  ...
  └─────────────┘                  ▼
               ...
               ▼                    ▼      ▼                         ▼
  ┌─────────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────────┐
  │  ADJUDICATION   │  │   FRAUD &    │  │  ANALYTICS   │  │  MEMBER/EMPLOYER │
  │  ENGINE         │  │   AUDIT SVC  │  │  PIPELINE    │  │  NOTIFICATION    │
  │                 │  │              │  │              │  │  SERVICE         │
  │ • Rules eval    │  │ • ML scoring │  │  Kafka       │  │                  │
  │ • Accumulator   │  │ • Anomaly    │  │  Connect →   │  │ • EOB push/email │
  │ • Network logic │  │   detection  │  │  Snowflake / │  │ • Status updates │
  │ • Payment calc  │  │ • Audit log  │  │  Databricks  │  │ • Cost alerts    │
  └────────┬────────┘  └──────────────┘  └──────────────┘  └──────────────────┘
           │
           ▼
  ┌─────────────────────────────────────────────┐
  │  PAYMENT SERVICE                             │
  │  • ACH file generation (NACHA)               │
  │  • Stop-loss accumulation feed               │
  │  • 835 remittance generation                 │
  │  → publishes payments_queued / settled       │
  └─────────────────────────────────────────────┘

  EVENT STORE (Kafka retention: 7-year compliance tier)
  ┌──────────────────────────────────────────────────────────────────────┐
  │  All events retained immutably → enables full claim replay for:      │
  │  • Audits  • Reprocessing  • Debugging  • FHIR EOB reconstruction    │
  └──────────────────────────────────────────────────────────────────────┘
```
...
Schema evolution is a persistent challenge in event-driven claims systems. The FHIR R4 Claim resource schema, the proprietary adjudication event schema, and the X12 837 parsing output schema all change over time. Apache Avro with a Schema Registry (Confluent Schema Registry or AWS Glue Schema Registry) is the standard approach for managing backward-compatible schema evolution in Kafka-based systems. Enforcing schema compatibility rules at the registry level prevents producers from publishing breaking changes that would corrupt downstream consumers.
...
Clinical NLP for unstructured data extends beyond coding. Prior authorization workflows benefit from NLP that extracts clinical criteria from submitted medical records. Grievance and appeal letters can be automatically triaged by clinical issue type. Lab report interpretation for high-cost condition identification is another use case. These applications are more tractable for internal development because the labeled dataset is your own operational data, not general clinical text.
...
```
ML PIPELINE FOR CLAIMS ANALYTICS
═══════════════════════════════════════════════════════════════════

  ┌─────────────────────────────────────────────────────────────┐
  │                    DATA INGESTION LAYER                     │
  │  837 Claims ──┐                                             │
  │  834 Eligib. ─┤──► Raw Kafka Topics ──► Schema Validation  │
  │  835 ERA    ──┘         (Avro)              + Dedup         │
  └──────────────────────────────┬──────────────────────────────┘
                                 │
                                 ▼
  ...
  │                  FEATURE ENGINEERING                        │
  │                                                             │
  │  Member History    Provider Patterns    Temporal Features   │
  │  ─────────────     ─────────────────    ────────────────    │
  │  • HCC scores      • Billing velocity  • Days since last   │
  │  • Cost quintile   • Specialty mix      PCP visit          │
  │  • Rx adherence    • Peer benchmarks   • Claim seasonality  │
  │  • ER frequency    • Geographic peers  • Policy age        │
  └──────────────────────────────┬──────────────────────────────┘
                                 │
                    ┌────────────┼────────────┐
                    ▼            ▼            ▼
             ┌──────────┐ ┌──────────┐ ┌──────────┐
             │Supervised│ │Unsuperv. │ │  NLP     │
             │  Models  │ │Anomaly   │ │ Coding   │
             │          │ │Detection │ │ Assist   │
             │XGBoost   │ │Isolation │ │BERT-base │
             │Risk Score│ │Forest    │ │ICD-10/   │
             │FWA Flags │ │Autoencod.│ │CPT map   │
             └────┬─────┘ └────┬─────┘ └────┬─────┘
                  └────────────┼────────────┘
                               ▼
  ┌─────────────────────────────────────────────────────────────┐
  ...
  │                                                             │
  │  ┌─────────────────┐   ┌─────────────────┐                 │
  │  │  FWA Work Queue │   │  Care Mgmt      │                 │
  │  │  (SIU Review)   │   │  Outreach List  │                 │
  │  └─────────────────┘   └─────────────────┘                 │
  │  ┌─────────────────┐   ┌─────────────────┐                 │
  │  │  Auto-Adj       │   │  Provider       │                 │
  ...
  └─────────────────────────────────────────────────────────────┘
                               │
                               ▼
              Model Registry (MLflow) + Drift Monitoring
              Retrain trigger: PSI > 0.2 or F1 drop > 5%
```
...
| Phantom Billing | Billing for services never rendered; fabricated patient encounters or procedures | Member denies receiving service on audit; provider location vs. member location mismatch; service during member hospitalization elsewhere | Cross-claim correlation (member was inpatient at Hospital A when provider B billed outpatient visit); geospatial anomaly detection | DME company bills for power wheelchairs for members who have no mobility diagnosis and live in walkable urban areas |
...
Supervised approaches work best when you have a meaningful labeled dataset of confirmed FWA cases. XGBoost and gradient boosting models consistently outperform neural networks on tabular claims data — the structured, sparse feature space with known semantics favors tree-based methods. Feature engineering is the critical differentiator: raw claim fields are insufficient. You need engineered features like "provider's ratio of billed-to-allowed vs. specialty peers," "member's count of providers seen in rolling 90 days," and "time delta between service date and claim submission." SMOTE and cost-sensitive learning handle class imbalance, but neither fully solves the problem when positive rate is below 0.1%.
...
(0-6mo) Quick Wins
        ...
        └─ Basic analytics dashboard (PMPM, MLR, auto-adj)
             ...
             (6-18mo) Core Modernization
                  ...
                  └─ Microservices carve-out (eligibility,         │
                     pricing, auth) from monolith                   │
                                                                   │
                          PHASE 3 ████████████████████████        │
...
RISK PROFILE                                                       │
        LOW ────────────────────── MEDIUM ─────────── HIGH ───────┘
        (No core system changes)  (API layer;         (Core engine
                                   data migration)     replacement)
```
...
Real-time claim processing requires event-driven architecture end-to-end: claims submitted via real-time 837 transaction or REST API, eligibility verified synchronously, clinical edits applied via rules engine in-process, and an adjudication decision returned within 5 seconds for clean claims. This is a massive architectural change from the batch-processing paradigm of legacy systems, but it's the future of the market — providers want to know at point of service whether a claim will pay.
...
| --- | --- | --- | --- | --- | --- |
| Auto-adjudication rate | 15% | Request 12-month auto-adj rate by claim type from 3 reference customers similar to your volume/mix. Refuse aggregate rates — get professional and institutional separately. | Vendor cannot provide reference customer data; rate below 75% on professional claims | __ | __ |
| API maturity and integration flexibility | 15% | Require a real API sandbox. Test actual EDI ingestion, real-time eligibility, and claim status endpoints. Count undocumented limitations. | No sandbox; REST "API" is actually SFTP file drop with JSON wrapper; rate limits incompatible with your volume | __ | __ |
...
| Plan design configurability | 12% | Can your team configure plan designs without vendor professional services? Test HSA-compatible HDHP, HRA-integrated plan, and tiered network benefit simultaneously. | Every plan design change requires a vendor ticket; configuration UI is COBOL screen forms; no self-service benefit administration | __ | __ |
| Data access and portability | 12% | Full data export on contract termination (within 30 days, in standard format), real-time data feed for your warehouse, no data licensing fees for your own data | Data export limited to PDF reports; termination data export takes 6+ months; proprietary data format with no standard export | __ | __ |
| Scalability and SLA | 10% | 99.9% vs 99.95% uptime SLA (the difference is 4x more downtime annually); claims processing SLA during open enrollment peaks; disaster recovery RTO/RPO | No financial penalties in SLA; "commercially reasonable efforts" language in uptime commitments; RTO > 24 hours | __ | __ |
| Regulatory compliance roadmap | 10% | When did they ship FHIR R4 Patient Access API? What is their timeline for CMS prior auth rule (January 2026)? Do they have a dedicated compliance engineering team? | FHIR compliance is "on roadmap" without timeline; compliance features ship after regulatory deadline, not before | __ | __ |
| Implementation track record | 8% | Require 5 customer references at your scale. Ask specifically: what was the go-live date vs. contracted date? What went wrong? | All references are enterprise customers when you're mid-market; no reference will give a candid implementation assessment | __ | __ |
| Total cost of ownership | 8% | PMPM pricing including professional services for plan design changes, integration work, and regulatory updates. Many vendors quote low PMPM and charge $300/hr for everything else. | Pricing structure requires professional services for standard configuration; "unlimited" plan design has 20-page definition of what's excluded | __ | __ |
| Vendor financial stability | 5% | Last fundraise date and amount, ARR growth trajectory, customer concentration (is one customer >25% of revenue?), key person dependency | Undisclosed funding; single large customer concentration; CTO/founder departure in last 12 months | __ | __ |
...
| Claims turnaround time | Median days from receipt to final payment/denial, by claim type | <14 days clean claims; <30 days pended; state prompt-pay compliance | Processing bottlenecks, manual queue backup, system performance issues, compliance risk | Daily |
...
Competitive advantage: Gravie's ICHRA-first model, Oscar's vertical integration, and Clover Health's data-driven provider partnerships are all technology stories. Your board needs to understand why your technology roadmap creates durable competitive advantage — not just operational efficiency. "Our AI-powered member experience will achieve NPS 20 points above industry average" is a board narrative. "We're rewriting the member portal in React" is not.
