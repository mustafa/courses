# Skills Wallet Architecture — Verifiable Credentials and What Coursera Should Build
...
[1The Credentialing Problem](#mod1) [2The Standards Stack](#mod2) [3The Trust Triangle](#mod3) [4Wallet Architecture](#mod4) [5The Issuer Side](#mod5) [6The Verifier Side](#mod6) [7The Cloudflare Wallets Intersection](#mod7) [8The Ecosystem](#mod8) [9Competitive Analysis](#mod9) [10Implementation Roadmap](#mod10) [11Technical Deep Dive](#mod11) [12Privacy & Compliance](#mod12)

## Module 1: The Credentialing Problem
...
> **Warning:** A number to get right before you put it in a deck
>
...
| --- | --- | --- |
| Commonly cited "digital credentials" figure | $1.13B | 21.7% |
...
**Q: What is the structural difference between a traditional certificate and a verifiable credential?**

-   Verifiable credentials are stored in a blockchain rather than a database
-   A traditional certificate is a pointer to an issuer-controlled database row; a verifiable credential is a self-contained signed statement the learner holds **(correct)**
-   Verifiable credentials cannot be forged, while PDFs can
-   Verifiable credentials are machine-readable and PDFs are not

*Explanation: The location of authority is the whole point. A PDF's verification link means the issuer's server is the source of truth, so the credential dies if the issuer changes URLs or disappears. A VC carries its own cryptographic proof, so it verifies against a public key without the issuer being in the loop. Machine-readability and forgery resistance are consequences, not the definition — and blockchain is not required at all.*

**Q: Credential Engine counted 1.85 million unique US credentials in 2025, with badges (1.02M) outnumbering degrees (264K) roughly four to one. What does this most directly imply for credential strategy?**

-   Issuing more credentials is the fastest path to market share
-   The scarce resource is no longer credentials but the ability to discriminate between them, making assessment provenance the differentiator **(correct)**
-   Badges will replace degrees within a decade
-   The market is saturated and new issuers cannot compete

*Explanation: When 1.85 million credentials exist, any individual credential's signal value collapses unless something distinguishes rigor. That makes machine-readable assessment provenance — proctored versus graded versus completion-only — the actual product, which is precisely what a PDF cannot carry and a VC can.*

**Q: Why should you be careful citing Coursera's '94% of employers would pay more' figure in a verifiable-credentials business case?**

-   The sample size was too small to be meaningful
-   It measures willingness to pay more for micro-credential holders, not for cryptographically verified credentials — bridging those two is an assumption **(correct)**
-   The survey only covered the United States
-   The figure is from 2024 and is now outdated

*Explanation: The survey (3,500+ respondents across seven countries) is solid, but it establishes a premium for micro-credentials as a category. No published study establishes a premium for verified-versus-unverified. Treating the bridge as proven is the fastest way to lose credibility with a skeptical executive — state it as the central commercial assumption instead.*

-   **The three failures of the PDF certificate** — (1) Unverifiable offline — the document carries no proof, only a link. (2) Siloed — credentials from four issuers means four formats and four portals, with no composite view. (3) Institution-controlled — the authoritative record lives in the issuer's database, so URL changes, deprecation, or shutdown silently kill it.
-   **The one-sentence framing of the whole shift** — A traditional credential is a pointer to a database row the issuer controls. A verifiable credential is a signed statement the learner controls. Wallets, DIDs, status lists, and selective disclosure are all consequences of that one move.
-   **Coursera Micro-Credentials Impact Report 2026 — the four numbers** — 94% of employers would offer higher starting salaries to micro-credential holders. 92% say those hires perform better in year one. 86% use skills-based hiring for entry-level roles (98% globally in some form). 3,500+ respondents across seven countries.
-   **Counting Credentials 2025 (Credential Engine)** — 1,850,034 unique US credentials from 134,000+ providers — up from 1.08M in 2022. Badges 1.02M, certificates 486,352, degrees 264,099. $2.34T invested annually in US education and workforce development.
-   **Why market-size figures range 10x** — Scope definition does all the work: 'digital badges' (~$0.25B) versus 'digital credential management software' ($2.6–3.1B) versus the commonly cited $1.13B/21.7% figure. None counts the value of the hiring decisions involved. For Coursera the TAM question is the wrong question — the right one is attribution to enrollment, completion, and renewal.
-   **What actually breaks the 'nobody verifies anyway' objection** — Cost and automation. When verification is a free function call rather than a phone call, the calculus flips. More decisively, an AI screening agent processing 20,000 applications verifies every claim because the marginal cost is a round trip. The verification consumer of 2028 is probably not a person.

## Module 2: The Standards Stack
...
| Method | How it resolves | Good for | Trade-off |
| --- | --- | --- | --- |
| did:web | HTTPS fetch of /.well-known/did.json at the domain | Institutional issuers. Trust inherits from the domain and its TLS certificate. | Centralized — if you lose the domain, you lose the identity. For Coursera this is a feature. |
| did:key | Nothing to fetch — the key is encoded in the identifier itself | Learners. Zero infrastructure, works offline, generated on-device in milliseconds. | No rotation. Key loss is identity loss. Static across verifiers, so it correlates. |
| did:jwk | Same idea as did:key, JWK-encoded | JOSE-native stacks | Same rotation limitation |
| did:ion, did:ethr, ledger methods | Distributed ledger lookup | Censorship resistance | Operational complexity, cost, latency, and a governance story you must defend |
...
**Q: What is the relationship between Open Badges 3.0 and W3C Verifiable Credentials?**

-   They are competing standards that solve the same problem
-   Open Badges 3.0 is a profile of W3C VCs — an OB 3.0 assertion is a VC with an additional JSON-LD context and type **(correct)**
-   Open Badges 3.0 wraps VCs in a proprietary 1EdTech envelope
-   W3C VCs were derived from Open Badges 3.0

*Explanation: 1EdTech rebuilt Open Badges on the W3C VC data model for 3.0, and developed CLR 2.0 alongside it so both share that model. There is no conversion step — you add the OB context and the OpenBadgeCredential type, and achievement-specific vocabulary becomes available inside a standard VC.*

**Q: Which DID method combination is recommended for a platform like Coursera, and why?**

-   did:ion for both issuer and learner, for censorship resistance
-   did:web for Coursera and did:key for learners — trust inherits from the domain, and learners need zero infrastructure **(correct)**
-   did:key for both, since it requires no hosting
-   did:web for both, so every party is resolvable over HTTPS

*Explanation: Coursera's trust genuinely derives from controlling coursera.org, so did:web (an HTTPS fetch of /.well-known/did.json) matches reality and lets verifiers reuse infrastructure they already have. Learners need an identifier generated on-device in milliseconds with nothing to host, which is did:key. A ledger adds cost and governance while removing nothing.*

**Q: A CLR 2.0 bundles credentials from three different issuers. What must a verifier do?**

-   Verify only the CLR's outer signature, since the bundler vouches for its contents
-   Verify the CLR's signature and independently verify each inner credential's own proof **(correct)**
-   Reject the CLR, since multi-issuer bundles are not permitted
-   Verify the inner credentials only if the outer signature fails

*Explanation: Bundling is assembly, not attestation. The CLR signature proves who assembled the set; it says nothing about the validity of credentials issued by other parties. Trusting the outer signature alone is the most common CLR implementation bug, and it is a security bug — it would let any bundler launder arbitrary claims.*

**Q: Which layer decision most constrains your options at every layer above it?**

-   The DID method chosen for learners
-   The securing mechanism at the VC layer, because it determines whether field-level selective disclosure is possible **(correct)**
-   The transport protocol used for issuance
-   The status list size

*Explanation: Signing with a plain Ed25519 signature over the whole document forecloses field-level disclosure for those credentials permanently — you cannot retrofit it without reissuing. DID method, transport, and status list size are all swappable later without re-signing. This is the one place where layer independence breaks down.*

-   **The six-layer stack, bottom to top** — L0 DIDs (identifiers) → L1 VC Data Model 2.0 (container + securing) → L2 Open Badges 3.0 (achievement semantics) → L3 CLR 2.0 (bundling) → L4 OpenID4VCI/VP (transport) → L5 Bitstring Status List (revocation).
-   **VC 2.0 traps for people with v1.1 habits** — validFrom/validUntil replaced issuanceDate/expirationDate. issuer can be a string or object. credentialSubject.id is optional — omitting it creates a bearer credential. proof isn't the only securing mechanism (VC-JOSE-COSE uses a JWT wrapper). @context is load-bearing: unknown terms are errors.
-   **The two OB 3.0 fields that carry strategic weight** — criteria (what the learner actually had to do — 'watched videos' vs 'passed a proctored exam at 80%' are the same shape, different meaning) and alignment (pointers to external skill frameworks like O\*NET, ESCO, or CTDL, which is what makes credentials comparable across issuers).
-   **Why alignment is the highest-leverage cheap win for Coursera** — Without alignment, badges from three issuers are three incomparable strings. With it, they point at the same skill concept and become a set operation. Coursera already maintains an internal skills taxonomy across its catalog — emitting it as alignment entries is cheap engineering that makes its credentials machine-comparable while competitors emit bare strings.
-   **CLR bundling does not re-attest** — A CLR containing third-party credentials means 'I assembled these,' not 'I vouch for these.' Verifiers must independently check every inner credential's own proof. Trusting only the outer signature is the most common CLR bug — and it's a security bug, since it would let any bundler launder arbitrary claims.
-   **did:web vs did:key** — did:web resolves by HTTPS fetch of /.well-known/did.json — trust inherits from the domain, supports rotation, right for institutions. did:key encodes the key in the identifier itself — zero infrastructure, works offline, but no rotation and it correlates across verifiers. Right for learners.
-   **The three genuinely unsolved problems** — (1) Trust registries — signature verification proves who issued, not whether they should be believed; anyone can stand up a convincing did:web. (2) Wallet portability — credentials are portable, whole wallets are not. (3) Key recovery — every fix trades away self-sovereignty or usability.

## Module 3: The Trust Triangle
...
Verifier → RegistryResolves `did:web:coursera.org` → `https://coursera.org/.well-known/did.json`. Cached aggressively.
...
> **Warning:** Step 4 is the one implementations skip
>
...
| Mechanism | How it works | Unlinkable? | Readiness |
| --- | --- | --- | --- |
| Atomic credentials | Issue many small credentials instead of one big one. Share only the relevant ones. | No | Ship today — needs no crypto beyond ordinary signatures |
| SD-JWT (RFC 9901, Nov 2025) | Issuer signs salted hashes of each claim; holder reveals only chosen claims plus their salts. | No — the signature is a constant across presentations | Production — an IETF RFC, used in EUDI |
| BBS+ / bbs-2023 | Multi-message signature scheme; holder derives a fresh zero-knowledge proof revealing a chosen subset. | Yes — each derived proof is unique | Emerging — cryptosuite still maturing, library support thin |
| ZK predicates ("score > 80" without the score) | General-purpose zero-knowledge circuits over credential claims. | Yes | Research — no interoperable production deployment in education |
...
> **Warning:** Where DIDComm stands versus OpenID4VC — say this accurately
>
...
**Q: What is the single property that most justifies the verifiable-credentials architecture?**

-   Credentials cannot be forged because they are cryptographically signed
-   The verifier never has to contact the issuer about a specific learner, eliminating a privacy leak, a latency cost, an availability dependency, and a business relationship at once **(correct)**
-   Credentials are stored on a decentralized ledger
-   Learners can revoke access to their credentials at any time

*Explanation: Forgery resistance is valuable but is the easy part. The architectural payoff is that verification requires only a cacheable, learner-independent public key fetch and a status list covering ~131,000 credentials — so nobody learns which learner was checked, and the issuer's availability is irrelevant to the transaction.*

**Q: A verifier checks the credential's signature against Coursera's public key and confirms it is unmodified and unrevoked. What critical check might still be missing?**

-   Whether the credential's JSON-LD context resolves
-   The presentation signature — proving the presenter actually controls the subject DID rather than merely possessing a copy **(correct)**
-   Whether the issuer's TLS certificate is valid
-   Whether the achievement has an alignment field

*Explanation: Skipping the presentation-signature check is a complete authentication bypass: you would have proven Coursera said something true about Alice, then handed the job to Bob. The nonce and domain binding matter equally — without them a captured presentation replays to a different verifier.*

**Q: Which selective-disclosure mechanism is production-ready AND provides unlinkability across presentations?**

-   SD-JWT, standardized as RFC 9901 in November 2025
-   Atomic credentials — issuing many small credentials instead of one
-   BBS+ (bbs-2023) — production-ready and unlinkable
-   None: SD-JWT is production-ready but linkable, and BBS+ is unlinkable but still maturing **(correct)**

*Explanation: SD-JWT ships today and is used in EUDI, but the issuer signature is a constant across presentations, so two verifiers can correlate. BBS+ derives a fresh zero-knowledge proof each time and is genuinely unlinkable, but cryptosuite and library support are still thin. There is currently no option that is both.*

**Q: Why should DIDComm not be your primary transport for credential issuance in 2026?**

-   It is insecure for high-value credentials
-   It has been deprecated by the Decentralized Identity Foundation
-   OpenID4VCI/VP won in practice — they are what EUDI standardized on and what the browser Digital Credentials API hardcoded at TPAC November 2025 **(correct)**
-   It cannot carry W3C Verifiable Credentials

*Explanation: DIDComm is technically elegant and still useful for peer-to-peer flows, but the ecosystem consolidated elsewhere. The W3C FedID WG hardcoded openid4vp-v1-\*, org-iso-mdoc, and openid4vci-v1 — DIDComm is not on that list. Choosing it as the main channel means building bridges to reach every wallet that matters.*

-   **The trust triangle, plus the corner people forget** — Issuer signs and publishes a public key, then leaves. Holder stores and decides what to share. Verifier checks proof and status. The fourth corner is the Verifiable Data Registry — for did:web that's just HTTPS + DNS + the CA system, not a blockchain.
-   **The eight verification steps** — (1) Request with nonce + domain. (2) Wallet matches, learner approves. (3) Holder signs a VerifiablePresentation. (4) Check presentation signature. (5) Resolve issuer DID. (6) Check credential signature. (7) Check status list bit. (8) Decide whether you trust the issuer — that last one is policy, not cryptography.
-   **The check implementations skip** — The presentation signature (step 4). Without it, anyone possessing a copy of a credential can present it as their own — you'd verify Coursera said something true about Alice, then hire Bob. Nonce freshness and domain binding are equally load-bearing against replay.
-   **What VCs actually prevent** — Forgery and tampering — a large, worthwhile subset. They do NOT prevent collusion: nothing stops a learner from handing over their phone and PIN. Holder binding makes credentials non-transferable only to the extent the holder guards their key.
-   **Selective disclosure, ranked by readiness** — Atomic credentials (ship today, linkable) → SD-JWT / RFC 9901 (production, used in EUDI, linkable) → BBS+ bbs-2023 (unlinkable, still maturing) → ZK predicates like 'score > 80' (research; no interoperable education deployment). Design for atomic, secure with SD-JWT, treat BBS+ as fast-follow, don't roadmap ZK predicates.
-   **When unlinkability actually matters** — Rarely for employment credentials — the learner is deliberately presenting under their real name to a named employer, so correlation is moot. It's decisive for anonymous skills marketplaces and credentials touching protected categories. Segment by sensitivity; that securing decision is near-irreversible at scale.
-   **Herd privacy in Bitstring Status List** — One compressed bitstring covers ~131,072 credentials; each credential holds an index; the verifier fetches the whole list and reads one bit. The issuer learns someone checked list #3, not which credential. Residual leaks: verifier IP/timing (serve from CDN, don't log) and small lists shrinking the anonymity set.

## Module 4: Wallet Architecture
...
|  | On-device only | Cloud-backed (encrypted) | Custodial / web wallet |
| --- | --- | --- | --- |
| Key custody | Learner, hardware-backed | Learner; encrypted backup escrowed | Provider holds keys |
| Device loss | Total loss | Recoverable | Nothing to lose |
| Multi-device | No | Yes | Yes |
| Provider can impersonate | No | No (if E2E) | Yes |
| Onboarding friction | App install | App install + backup setup | A login |
| Honest label | Self-sovereign | Self-sovereign with a safety net | Not self-sovereign — say so |
...
**Q: Why must a learner's private key be generated on-device and never transmitted?**

-   Network transmission is slow and would hurt onboarding conversion
-   A key that touched a server could have been retained, meaning the issuer could impersonate learners — which makes the self-sovereignty claim theater **(correct)**
-   Regulations prohibit transmitting cryptographic material
-   Server-generated keys are cryptographically weaker

*Explanation: The security property being claimed is that only the learner can present their credentials. If Coursera ever held the key, Coursera could forge presentations, and no verifier could tell the difference. Hardware-backed non-extractable storage extends this: malware can request signatures on the device but cannot exfiltrate the key and sign elsewhere forever.*

**Q: What is the pragmatic answer to key recovery for a large education issuer?**

-   Seed phrases, since they are fully self-sovereign
-   Mandatory social recovery with designated guardians
-   Hardware-backed keys plus optional encrypted backup, with frictionless reissuance designed as a first-class flow **(correct)**
-   Custodial key storage for all learners

*Explanation: A decade of crypto-wallet evidence shows ordinary users lose seed phrases, and almost nobody designates guardians in advance. At Coursera's scale, reissuance will be the dominant recovery path by an enormous margin, so it should be a designed flow rather than a support escalation. It does quietly reintroduce issuer-side records — which is fine, as long as nobody pretends otherwise.*

**Q: What is the main lesson from ASU Pocket's deployment?**

-   Native mobile wallets outperform web wallets on adoption
-   Blockchain-based self-sovereign identity is required for university deployments
-   ASU shipped a wallet plus an issuer network plus a target population with a concrete use case — the wallet was the least interesting part **(correct)**
-   Open beta launches should precede standards compliance

*Explanation: ASU paired ASU Pocket with the Trusted Learner Network and a focused career-mobility application (My Skills Pocket for retail workers). Funding the app while underfunding issuance and demand reproduces the field's most common failure: a well-built wallet with nothing worth putting in it.*

**Q: What is the contrarian recommendation about Coursera building a native wallet?**

-   Build it first, since credential storage is the core learner value
-   Build nothing and rely entirely on Apple and Google wallets
-   Ship a lightweight custodial web wallet for reach, invest heavily in standards-compliant issuance and export, and defer the native app until there is evidence of demand **(correct)**
-   Partner with ASU to white-label ASU Pocket

*Explanation: A native wallet is a permanent product line — roughly 3–4 engineers steady-state, two app stores, OS churn, and a protocol treadmill — competing against Apple, Google, and EUDI national wallets. The defensible asset is being the best issuer: credentials so well-structured and richly skill-aligned that every wallet wants them and every verifier trusts them.*

-   **The six wallet components** — Key store (hardware-backed, non-extractable), credential store (encrypted local DB), protocol handlers (OID4VCI/VP + deep links/QR), presentation engine (match, disclose, sign), backup & recovery, and trust/policy UI. Three are solved; recovery is why wallets are still bad.
-   **Key management non-negotiables** — Generate on device and never transmit. Hardware-backed and non-extractable (Secure Enclave / StrongBox). Bind signing to biometrics or passcode. Consider per-verifier pairwise DIDs, since one static did:key across every employer is a cookie that can't be cleared.
-   **The four recovery options and their costs** — Reissue from issuer (works; quietly reintroduces the issuer-controlled DB). Encrypted cloud backup (good UX; passphrase becomes the SPOF). Seed phrase (sovereign; catastrophic usability). Social recovery (elegant; requires guardians nobody designates). Pick reissuance and design it as a first-class flow.
-   **MIT Learner Credential Wallet** — Built by the Digital Credentials Consortium (US DOE grant funding from 2020), iOS + Android, supports W3C VCs and Open Badges 3.0. Stewardship transferred to the OpenWallet Foundation (Linux Foundation); code at openwallet-foundation-labs/learner-credential-wallet. Education-native, open source, shipped, neutrally governed.
-   **ASU Pocket** — ASU's VC wallet for badges, experiences, academic and employment records, encrypted on device. Open beta iOS + Android November 2023, paired with the Trusted Learner Network and My Skills Pocket for retail-worker career mobility. The organizational lesson beats the technical one.
-   **The honest custodial-vs-self-sovereign call** — A custodial web wallet will out-adopt a self-sovereign mobile wallet roughly 10x, because 'log in' beats 'install, secure a key, save a backup.' Ship the custodial web wallet for reach, support export to any compliant wallet for sovereignty, and never market custodial as self-sovereign.
-   **Wallet-side revocation** — Wallets don't enforce revocation — verifiers do. But a good wallet re-checks status lists in the background and warns the learner before an interview does. Cost: a device-linked traffic pattern. Mitigate by batching, randomizing timing, sharing a CDN, and letting learners disable it.

## Module 5: The Issuer Side — What Coursera Needs to Build
...
Trigger**Completion event.** A learner finishes a course, specialization, or professional certificate. Emitted onto a durable queue — never issued synchronously in the request path.

2

Gate**Eligibility & integrity check.** Grades finalized? Honor-code flags clear? Payment settled? Refund window closed? This gate is what makes the credential worth signing — and the source of nearly every "why was I not issued" ticket.

3

Assemble**Claim assembly.** Pull achievement definition, skill alignments, assessment metadata, instructor, partner institution, effort hours. Resolve the learner's subject DID (or mint a placeholder if they have no wallet yet).

4

Validate**Schema validation.** Validate against OB 3.0 before signing. A signed invalid credential is worse than no credential — it is a permanent, publicly verifiable bug.

5

Allocate**Status list index.** Reserve an index in the current bitstring status list. Persist the mapping durably before signing, or you lose the ability to revoke.

6

Sign**Signing service.** The only component with access to the private key. Takes a validated document, returns a secured credential. Small, boring, heavily audited, separately deployed.

7

Deliver**Offer generation.** Create an OID4VCI credential offer, plus a hosted fallback URL and a downloadable file. Not every learner has a wallet, and the ones who do not are the majority.

8

Notify**Learner notification.** Email/in-app: what they earned, what it proves, how to claim it into a wallet, and how to share it. The step that determines whether any of the previous seven mattered.
...
| Tier | What it means | Evidence carried | Employer read |
| --- | --- | --- | --- |
| T4 — Proctored | Identity-verified, proctored, timed assessment | Proctoring provider, identity-verification method, score band | Strongest — comparable to a certification exam |
| T3 — Auto-graded | Machine-graded programming assignments and quizzes with a pass threshold | Assignment count, threshold, weighted average band | Strong — demonstrates applied capability |
| T2 — Peer-reviewed | Human-reviewed submissions against a rubric | Rubric reference, reviewer count | Moderate — good for open-ended work, noisier signal |
| T1 — Completion | Content consumed; no meaningful assessment | Effort hours, modules completed | Participation — exposure, not capability |
...
| Stage | Cost per credential | Bottleneck? |
| --- | --- | --- |
| Ed25519 signature | Tens of microseconds | No — a single core does tens of thousands/sec |
| JSON-LD canonicalization (RDFC-1.0) | Single-digit to tens of milliseconds | Yes — often 100–1000x the signing cost |
| Context resolution over the network | Unbounded | Catastrophic if not pre-cached — and a liveness dependency on third-party hosts |
| Claim assembly (DB reads) | Milliseconds | Manageable with batching |
| Status list allocation | Microseconds, but contended | Needs care — pre-allocate index ranges per worker |

> **Warning:** The counter-intuitive result: the crypto is free, the JSON-LD is not
>
...
| Model | Who signs | Verifier sees | When to use |
| --- | --- | --- | --- |
| Coursera as sole issuer | did:web:coursera.org | Coursera, with the partner named in the achievement | Default. Simplest, one key, one trust decision. |
| Sub-issuer DID | did:web:coursera.org:issuers:asu | A Coursera-hosted identity clearly scoped to the partner | Partner wants distinct identity without running infrastructure. The sweet spot. |
| Partner as issuer, Coursera as delivery | did:web:asu.edu | ASU | Accredited award where the institution must be the attesting party. Requires partner key custody. |
| Dual credentials | Both, separately | Two linked credentials | Both parties assert different things — Coursera the assessment, the institution the academic award. |
...
**Q: Why must retired signing keys remain in the DID document forever?**

-   W3C DID Core requires that verification methods be immutable
-   Removing a retired key makes every credential ever signed with it unverifiable — which reads to a verifier as forged, not as expired **(correct)**
-   DID documents cannot be modified after publication
-   It allows the issuer to re-sign old credentials without reissuing

*Explanation: This is the trap that bites once and hurts enormously: rotating a signing key like a TLS certificate would break tens of millions of historical credentials in one deploy. You stop signing with a retired key but keep publishing its public half. The same logic means removing a compromised key is not the fix either — revoke the fraudulent credentials via the status list instead.*

**Q: In a Data Integrity issuance pipeline at scale, what is the actual bottleneck?**

-   Ed25519 signature generation
-   JSON-LD canonicalization (RDFC-1.0), often 100–1000x the cost of the signature **(correct)**
-   Status list index allocation
-   Database reads during claim assembly

*Explanation: Signing is tens of microseconds — a single core handles tens of thousands per second. Canonicalization is single-digit to tens of milliseconds. Worse, resolving @context URLs over the network is unbounded and is also a security problem, since a remote context is effectively remote code execution over your semantics. Pin contexts locally, or consider JWT/SD-JWT securing which skips canonicalization entirely.*

**Q: What is the recommended approach for backfilling credentials for millions of historical completions?**

-   Issue all credentials immediately as bearer credentials and bind them later
-   Lazy issuance: precompute an eligibility-and-tier index, then sign only when a learner claims **(correct)**
-   Issue only to learners who already have a wallet
-   Skip backfill entirely and issue going forward only

*Explanation: Lazy issuance avoids minting tens of millions of credentials nobody claims, avoids an unbound bearer-credential population, binds each credential to a real subject DID at creation, and lets you fix schema mistakes before most credentials exist. The cost is a few hundred milliseconds of claim latency that nobody notices.*

**Q: Why should Coursera publish an assessment-integrity tier that explicitly includes a weak 'completion-only' level?**

-   Regulatory disclosure requirements mandate it
-   Because the issuer who tells you which of their credentials are weak is the issuer you believe about the strong ones — legibility is the moat **(correct)**
-   It reduces liability for credential misrepresentation
-   It allows charging different prices per tier

*Explanation: In a market with 1.85 million credentials, an employer who can filter for proctored and auto-graded credentials gets a high-precision pool and a concrete reason to prefer Coursera over issuers emitting undifferentiated badges. A competitor cannot copy this quickly because it requires actually having the assessment data. Keep the taxonomy under academic-integrity governance, not marketing.*

**Q: What does the sub-issuer DID pattern (did:web:coursera.org:issuers:asu) give a university partner?**

-   Full custody of their own signing keys with no Coursera involvement
-   A distinct issuer identity with its own keys, hosted by Coursera, requiring zero partner infrastructure **(correct)**
-   The ability to revoke Coursera-issued credentials
-   A separate blockchain anchor for their credentials

*Explanation: did:web supports path-based identifiers, so did:web:coursera.org:issuers:asu resolves to https://coursera.org/issuers/asu/did.json. The partner gets branding and identity separation while key custody stays in one operationally mature place. Partners who genuinely must be the attesting party for accreditation reasons need the full partner-as-issuer model instead.*

-   **The eight-stage issuance pipeline** — Completion event → eligibility/integrity gate → claim assembly → schema validation → status list index allocation → signing service → OID4VCI offer generation → learner notification. Never issue synchronously in the request path.
-   **Separate assembly from signing** — Assembly is complex, changes often, touches many systems. Signing is simple, changes rarely, touches a private key. Combining them means every schema change is a change to key-access code requiring security review. Keep the signing service a few hundred lines: validated document + key ID in, secured credential out.
-   **The key rotation rule** — Never delete a verification method from the DID document. Stop signing with retired keys but keep publishing their public halves forever, or every credential signed with them becomes unverifiable — which reads as forged. Compromise is handled by revoking the fraudulent credentials, not by removing the key.
-   **The four assessment integrity tiers** — T4 Proctored (identity-verified, timed) → T3 Auto-graded (machine-graded with pass threshold) → T2 Peer-reviewed (rubric, human reviewers) → T1 Completion (content consumed, no meaningful assessment). Machine-readable and filterable, not buried in prose.
-   **Batch issuance cost model** — Ed25519 signing: tens of microseconds, not the bottleneck. JSON-LD canonicalization: 100–1000x that. Remote @context fetch: unbounded and a security hole. Fixes: pin contexts locally, consider JWT/SD-JWT securing to skip canonicalization, and pre-allocate status-list index ranges per worker.
-   **Backfill date semantics** — validFrom is when the credential became valid (2026); awardedDate on the subject is when the achievement happened (2019). Conflating them produces credentials that look backdated, which reads as fraud.
-   **The four partner issuance models** — Coursera sole issuer (default). Sub-issuer DID did:web:coursera.org:issuers:asu (the sweet spot — partner identity, zero partner infra). Partner as issuer did:web:asu.edu (needed when accreditation requires the institution to attest). Dual credentials (each party asserts different things).
-   **The Coursera for Business wedge** — Not individual badges — a verifiable workforce skills record (CLR) an enterprise holds for its own workforce, portable into their HRIS and re-verifiable without trusting a Coursera dashboard. Moves the sale from L&D ('employees will learn things') to compliance, workforce planning, and M&A diligence — a different budget line that outlives the subscription.

## Module 6: The Verifier Side — What Employers See
...
| # | Check | Network? | Typical cost |
| --- | --- | --- | --- |
| 1 | Presentation signature valid, nonce fresh, domain matches | No | Microseconds |
| 2 | Resolve issuer DID → public key | Yes, cacheable for hours | ~50 ms cold, 0 warm |
| 3 | Credential signature valid against that key | No | Microseconds |
| 4 | Status list bit not set | Yes, one fetch per ~131k credentials | ~50 ms cold, 0 warm |
| 5 | validFrom / validUntil window, schema conformance | No | Microseconds |
| 6 | Do I trust this issuer? | Policy lookup | Depends entirely on your trust model |
...
> **Warning:** Check 6 is the whole ballgame, and it is not cryptography
>
...
| Integration pattern | How it works | Effort | Fidelity |
| --- | --- | --- | --- |
| Verification receipt attachment | Candidate verifies via a hosted page; a signed PDF/JSON receipt attaches to the application | Low | Low — a human reads it |
| Custom field population | An integration writes verified skills into ATS custom fields the recruiter can filter on | Medium | Medium — filterable but lossy |
| Marketplace app | A native app in the Workday/Greenhouse marketplace performing verification in-flow | High | High — but per-ATS build and certification |
| Velocity Network relay | Publish into an HR-industry network that ATS vendors already integrate with | Medium | High — if the network has real reach |
...
**Q: In steady state, what does verifying a credential actually cost a verifier?**

-   One API call to the issuer per credential
-   Two aggressively cacheable fetches (issuer DID document and status list) plus local elliptic-curve math — sub-millisecond warm **(correct)**
-   A blockchain query with variable latency
-   A signed request to a trust registry for each check

*Explanation: The issuer DID document is learner-independent and cacheable for hours; one status list fetch covers ~131,000 credentials. Everything else is local signature math. That collapse in cost is what turns verification from a procurement decision into a line in a function.*

**Q: Which verification check has no standardized answer and stalls most real deployments?**

-   Validating the credential's JSON schema
-   Checking the revocation status list
-   Deciding whether to trust the issuer at all — check 6 is policy, not cryptography **(correct)**
-   Verifying the holder's presentation signature

*Explanation: Checks 1–5 are mechanical and a library handles them. 'Should I believe did:web:coursera.org?' has only unsatisfying answers today: hardcoded allowlists, 1EdTech certification as proxy, accreditation lookups, or simply recognizing the domain. That last one is doing most of the real work, which is why did:web suits institutional issuers — it reuses a trust judgment verifiers already make.*

**Q: Why should Coursera publish an open-source verifier library that performs all checks locally without calling Coursera?**

-   It reduces infrastructure costs for running a verification API
-   Because ubiquity of verification is worth more than revenue from it — a credential's value scales with how many parties can check it without a prior relationship **(correct)**
-   Open standards legally require reference implementations
-   It prevents competitors from building their own verifiers

*Explanation: Every gate in front of verification reduces the number of parties who can check a credential, which reduces the credential's value. The standards are open, so verification is not a defensible chokepoint anyway — someone else will publish the library. Charge for enrichment (skills-graph context, cohort percentiles, bulk operations) rather than for the basic check.*

**Q: Why should Coursera avoid shipping a single composite 'trust score'?**

-   Composite scores are computationally expensive at scale
-   It is unauditable for rejected candidates, becomes an optimization target within a quarter, and makes Coursera the arbiter of candidate quality with legal exposure under AI-hiring rules **(correct)**
-   Employers prefer binary verified/unverified indicators
-   The W3C VC specification prohibits derived scores

*Explanation: A candidate rejected because of an opaque 87 cannot contest the weighting, and automated employment decisions carry explainability and bias-audit obligations in several jurisdictions (NYC Local Law 144 among them). Expose the individual signals — assessment depth, issuer standing, recency, cohort context — and keep the weighting on the verifier's side of the line.*

**Q: What is the recommended first ATS integration pattern, and why?**

-   A native Workday marketplace app, since it has the highest fidelity
-   The verification receipt attachment — it works with every ATS ever built, needs zero partner engineering, and generates the usage data to justify deeper work **(correct)**
-   Velocity Network relay, since it reaches all ATS vendors at once
-   Direct OID4VP integration into each ATS

*Explanation: ATSs do not speak Verifiable Credentials natively. A signed receipt attaches to an application like any other document, so it works everywhere immediately. The trap is building a beautiful OID4VP integration nobody can consume because the ATS has no place to put the result — the bottleneck is the recruiter's screen, not the protocol.*

-   **The six verification checks** — (1) Presentation signature + nonce + domain. (2) Resolve issuer DID (cacheable hours). (3) Credential signature. (4) Status list bit (one fetch per ~131k credentials). (5) Validity window + schema. (6) Do I trust this issuer? — the only one that is policy rather than cryptography.
-   **Four verification API design rules** — Never return a bare boolean — return every check individually for compliance logging. Separate warnings from failures (a retired-key signature is a warning). Keep it stateless and key-free, or you rebuild the business relationship the architecture eliminated. Publish an open-source client that doesn't call you.
-   **Why give away verification** — A credential's value is proportional to how many parties can check it without a prior relationship with the issuer. Every gate reduces that number. The standards are open, so it isn't a defensible chokepoint anyway. Charge for enrichment — skills-graph context, cohort percentiles, bulk, provenance detail.
-   **The four trust signals to expose (not to combine)** — Assessment depth (T1–T4 tier), issuer standing (accreditation, 1EdTech certification, registry presence), recency and decay, and cohort context ('top decile of 340,000' — opt-in and selectively disclosable). Expose them separately; let the verifier weight them.
-   **Four ATS integration patterns, ranked by sequencing** — Verification receipt attachment (low effort, works everywhere — start here). Custom field population (medium, filterable but lossy). Marketplace app for Workday/Greenhouse (high effort, high fidelity, go where volume is). Velocity Network relay (a parallel bet on industry consolidation, not the primary path).
-   **Coursera's three demand-side levers** — C4B customers are employers with an existing relationship and a reason to verify their own workforce first. Hiring partners and job-board surfaces can require verified credentials, creating learner pull. Google Career Certificates are delivered on Coursera, so a large partner adopting drags its employer network along. Sequence: verify inside enterprises first, hiring second.

## Module 7: The Cloudflare Wallets Intersection
...
|  | Skills Wallet | Cloudflare Wallet |
| --- | --- | --- |
| Holds | Verifiable Credentials | Spending authority over funds |
| Owner | A learner | A human account owner, delegating to agents |
| Standards | W3C VC 2.0, OB 3.0, OID4VP | x402 over HTTP, Web Bot Auth |
| Answers | "Can I prove I have this skill?" | "Can an agent pay for one request?" |
| Status (Aug 2026) | Specs final; issuance scaling | Sell side (Monetization Gateway) live since ~July 2026; buy side largely announced |
...
Agent → Worker`POST /v1/verify/enriched` with a Verifiable Presentation. No credentials, no account, no prior relationship.

2

Worker → Agent`402 Payment Required` with a `PAYMENT-REQUIRED` header carrying an `accepts` array: schemes, networks, assets, and amounts the endpoint will take.
...
Worker → Agent`200 OK` with the enriched result and a `PAYMENT-RESPONSE` receipt.
...
| Scenario | Daily paid verifications | At $0.05 | Annual | Plausibility |
| --- | --- | --- | --- | --- |
| Today's world (humans checking at offer stage) | ~1,000–10,000 | $50–$500 | $18K–$180K | Near-certain — and not a business |
| Enterprise skills inventory (C4B workforce audits) | ~50,000 | $2,500 | ~$900K | Plausible — but better sold as a subscription |
| Agent-screened hiring (every application verified) | ~1M | $50,000 | ~$18M | Speculative — requires agentic screening at scale |
| Ambient verification (every skill claim, everywhere) | ~10M+ | $500,000 | ~$180M | Fantasy without an ecosystem shift |
...
**Q: Why must basic credential verification remain free and unkeyed?**

-   Cloudflare Workers cannot authenticate requests at the edge
-   Because a verifier facing a toll chooses the issuer without one — and since the standards are open, anyone can implement the checks locally anyway, so gating earns nothing and costs adoption **(correct)**
-   W3C requires free verification for conforming issuers
-   Free verification is required for GDPR compliance

*Explanation: Charging to verify a credential you already issued and were paid for tells the ecosystem your credentials carry a toll. Since checks 1–5 can be implemented from published open standards against your public DID document and status lists, gating is not even enforceable. Charge for enrichment — skills-graph context, cohort percentiles, provenance, bulk — which is genuinely Coursera-only data.*

**Q: In the x402 flow, what does the facilitator abstraction let a verification Worker avoid?**

-   Verifying the credential's cryptographic proof
-   Running a blockchain node, managing gas, and holding a private key — verify and settle become ordinary HTTP calls **(correct)**
-   Caching the issuer's DID document
-   Handling the 402 response itself

*Explanation: The Worker calls /verify and /settle and gets JSON back. It never touches a chain. That is what makes the protocol adoptable by an ordinary backend team, and it is the role Cloudflare plays at the edge in its own stack via the Monetization Gateway.*

**Q: A verification endpoint prices at amount '50000' with asset USDC. What is the price?**
...
*Explanation: Amounts are strings in the asset's smallest unit, and USDC has six decimals — so 50000 base units is 0.05 USDC, five cents. This off-by-six is a classic source of bugs; assert on it in tests.*

**Q: What is the honest assessment of x402 micropayment revenue from credential verification?**

-   It is a near-term $18M business at one million verifications per day
-   The reachable enterprise segment is better monetized as a subscription; x402 is the right architecture for the speculative agent segment, so build the endpoint but keep its revenue out of next year's plan **(correct)**
-   It will exceed subscription revenue within two years
-   It is not worth building until agentic screening is proven

*Explanation: Today's human-driven verification volume is thousands per day — not a business. Enterprise workforce auditing is reachable but enterprises hate unpredictable per-call billing and it belongs in a C4B contract line. The million-per-day agent scenario depends on employers deploying AI screening agents, a plausible 2027–2028 development rather than an August 2026 fact. Build it because marginal cost is near zero and it positions you; don't forecast it.*

-   **The one point where the two wallets intersect** — Verification is an API call, and API calls can be priced. Skills Wallet holds Verifiable Credentials (W3C VC 2.0, OB 3.0, OID4VP); Cloudflare Wallet holds delegated spending authority (x402, Web Bot Auth). Everything in Module 7 follows from that single sentence — including its limits.
-   **The x402 verification flow** — Request → 402 with PAYMENT-REQUIRED (accepts array) → agent signs authorization → retry with PAYMENT-SIGNATURE → Worker calls facilitator /verify then /settle → Worker verifies the credential → 200 with result plus PAYMENT-RESPONSE receipt. The 402 is a first-contact cost, not a per-call cost, since PAYMENT-REQUIRED is cacheable per route.
-   **The revenue scenarios, honestly ranked** — Today's human verification ~1–10k/day = $18–180K/yr (not a business). Enterprise skills inventory ~50k/day ≈ $900K (better as a subscription). Agent-screened hiring ~1M/day ≈ $18M (speculative, needs agentic screening at scale). Ambient verification ~10M/day ≈ $180M (fantasy without an ecosystem shift).
-   **The rule that must not be broken** — Basic verification is free forever — no key, no account, no rate limit a legitimate verifier would notice. Charging to verify a credential you already issued and were paid for is trust-destroying, and it isn't even enforceable since the standards are open. Charge for enrichment instead.
-   **Why Cloudflare Workers for verification** — DID documents and status lists are static artifacts fetched by every verifier — ideal CDN objects, and edge-serving also hides verifier IPs (the Module 3 privacy mitigation). Verification is stateless and CPU-light with no DB in the hot path. Global latency matters for a majority-non-US learner base. x402 is native at the edge since the July 2026 Monetization Gateway.
-   **The real prize — distribution, not payments** — Make the credential agent-legible: structured, skill-framework-aligned, verifiable without a human, coherently priced at the enrichment boundary. If an agent screening 20,000 applications finds Coursera credentials trivially machine-verifiable while everyone else's need a human to squint at a PDF, the format advantage compounds long before the revenue does.

## Module 8: The Ecosystem
...
| Organization | Owns | Why it matters to Coursera | Posture |
| --- | --- | --- | --- |
| W3C | VC Data Model 2.0, DIDs, Bitstring Status List, Digital Credentials API | The container everything else builds on. VC 2.0 became a Recommendation 15 May 2025. | Follow |
| 1EdTech | Open Badges 3.0, CLR 2.0, and the certification program | The gate that matters in education. Certification is how partners and buyers judge conformance. | Join & certify |
| OpenID Foundation | OID4VCI 1.0 (final Sept 2025), OID4VP 1.0 (final 10 July 2025), SIOPv2 | The transport every wallet and the EU stack speaks. | Follow |
| IETF | SD-JWT (RFC 9901, Nov 2025) | The practical selective-disclosure mechanism. | Follow |
| OpenWallet Foundation (Linux Foundation) | Stewards the Learner Credential Wallet and related OSS | Neutral home for wallet code — forking here is defensible with partners. | Contribute |
| Digital Credentials Commons (formerly DCC) | University consortium; built the LCW with US DOE funding from 2020 | Where Coursera's degree partners already are. | Engage |
| Velocity Network Foundation | The "Internet of Careers" — a public permissioned network for career credentials | HR-side interoperability; members hold career data on 1B+ individuals. | Evaluate |
| Credential Engine | Credential Registry and CTDL; Counting Credentials research | Where a credential becomes publicly discoverable and describable. | Publish into |
...
| Player | Position | OB 3.0 status | Relevance |
| --- | --- | --- | --- |
| Credly by Pearson | The largest badge network — 3,700+ issuers, ~200,000 credentials in catalog, 85% of top IT certifications | Certified 27 Jan 2026 | The default incumbent. Also powers the Google Cloud Credential Wallet. |
| Accredible | Certificates and badges for education and associations | OB 3.0 | Common in higher-ed procurement bake-offs |
| Canvas Credentials (Instructure, formerly Badgr) | Badging tied to the dominant higher-ed LMS | OB 3.0 | LMS-adjacent distribution — credentials issued where coursework already lives |
| Territorium | CLR-first learner records | CLR 2.0 + OB 3.0 certified | One of the few with real CLR 2.0 implementation experience |
| POK | Blockchain-anchored credentials | OB 3.0 | Ledger-anchoring niche |
| ASU / Trusted Learner Network | Institution running its own issuance + wallet | VC-native | The reference deployment for a university-scale program |

> **Warning:** Note who is not on this list: LinkedIn
>
...
**Q: What is the architectural relationship between Credential Engine's registry and Verifiable Credentials?**

-   They are competing approaches to credential verification
-   Credential Engine describes credential types (what a certificate requires and maps to); VCs assert instances (this person earned it) — they are complementary **(correct)**
-   Credential Engine issues the VCs on behalf of providers
-   VCs replaced the registry model after W3C VC 2.0 reached Recommendation

*Explanation: The registry plus CTDL says a credential type exists and what it means; a VC says a specific person earned it. Pairing them is powerful because a verifier resolving an achievement URI into a registry description gets independent third-party context on the credential's meaning — from someone other than the issuer.*

**Q: Why is the EU's December 2026 EUDI Wallet mandate the clearest forcing function in this space?**

-   It requires all EU universities to issue verifiable credentials
-   It breaks the chicken-and-egg problem by putting a credential wallet on hundreds of millions of phones by regulation, in one of Coursera's largest markets **(correct)**
-   It bans PDF certificates across the EU
-   It creates a single EU-wide credential registry

*Explanation: All 27 member states must offer a compliant wallet by December 2026. That eliminates the 'no wallets so no point issuing / no credentials so no point installing' deadlock by law. It also strengthens the case for deprioritizing a native Coursera wallet, and makes EDC/ELM mappability a requirement rather than a nice-to-have — proprietary formats aren't admissible into a EUDI wallet.*

**Q: What is the recommended posture toward Velocity Network Foundation?**

-   Join as a full member immediately and architect the credential pipeline around it
-   Ignore it — blockchain-based career networks have no future
-   Engage as observer, publish in standard formats a relay can pick up, and let membership follow demonstrated ATS pull rather than lead it **(correct)**
-   Build a competing network with Coursera's enterprise customers

*Explanation: Velocity's core insight is right — the bottleneck is HR-side, not education-side, and its members hold career data on 1B+ individuals. But much of the public material dates from 2020–2022, so verify current membership and live volume before committing, since a network's entire value is its reach. Publish standards-compliant credentials that a relay can consume, and don't architect around it.*

**Q: Why pursue 1EdTech certification rather than relying on conformance alone?**

-   Certification is legally required to issue Open Badges 3.0
-   Conformance makes credentials work; certification is a listed, checkable status that ends procurement conversations — and the test suite doubles as a conformance harness **(correct)**
-   Only certified issuers can publish to the Credential Registry
-   Certification grants access to 1EdTech's verification infrastructure

*Explanation: In education procurement the difference is decisive: when a university or state workforce agency asks whether something is OB 3.0 compliant, a certification listing ends the conversation and an engineer's assurance does not. Credly certified 27 January 2026 and Territorium holds both CLR 2.0 and OB 3.0 — in a bake-off, an uncertified competitor is answering questions the certified ones are not.*

-   **Which bodies to join versus follow** — Join and certify: 1EdTech (the gate that matters in education). Contribute: OpenWallet Foundation. Engage: Digital Credentials Commons (where degree partners already are). Publish into: Credential Engine. Follow: W3C, OpenID Foundation, IETF. Evaluate cautiously: Velocity Network.
-   **Velocity Network Foundation — the measured read** — Right insight: the bottleneck is HR-side, not education-side, and members hold career data on 1B+ individuals. Cautions: much public material dates from 2020–2022 (verify current reach), and the ledger adds governance you don't otherwise need. Engage as observer, publish standards-compliant credentials, let membership follow ATS pull.
-   **Credly by Pearson** — The largest badge network — 3,700+ issuers, ~200,000 credentials in catalog, 85% of top IT certifications. Certified for Open Badges 3.0 on 27 January 2026. Also powers the Google Cloud Credential Wallet. The default incumbent in any procurement bake-off.
-   **Registry vs credential — the distinction** — Credential Engine's registry + CTDL describes credential TYPES (what this certificate requires, what skills it maps to). A VC asserts an INSTANCE (this person earned it). Complementary: a verifier resolving the achievement URI gets third-party context on meaning from someone other than the issuer.
-   **US DOE Connecting Talent to Opportunity Challenge** — $15M prize competition inviting state Governors to build or scale Talent Marketplaces combining a credential registry, Learning and Employment Records, and skills-based job-description generators. Three phases, up to 10 semifinalists/finalists. Federal money creating verifier demand at the state level.
-   **EU stack: EDC, ELM, EUDI** — European Digital Credentials for Learning = the format for diplomas, certificates, micro-credentials. European Learning Model = the multilingual vocabulary describing them. EUDI Wallet = mandatory in all 27 member states by December 2026 (readiness varies, deadline is law). The clearest date-certain business case in the space.
...
| Dimension | LinkedIn | A verifiable Skills Wallet |
| --- | --- | --- |
| Distribution | Overwhelming | Starting from zero |
| Recruiter workflow integration | It is the workflow | None |
| Cryptographic verification | None — claims are self-reported | Native |
| Assessment provenance | Absent | Machine-readable tier + evidence |
| Learner ownership | LinkedIn-owned profile | Learner-held credential |
| Cross-platform portability | Locked in | Standards-based |
...
**Q: What is the correct strategic framing of verifiable credentials relative to LinkedIn?**

-   Verifiable credentials will displace LinkedIn profiles as the primary hiring surface
-   Verifiable credentials are a trust layer that can sit underneath any surface including LinkedIn's — the goal is to make the profile entry checkable, not to replace the profile **(correct)**
-   LinkedIn and verifiable credentials serve entirely separate markets
-   Coursera should build a competing professional network

*Explanation: A self-reported certification on LinkedIn influences more hiring decisions today than every verifiable credential ever issued, because it is where the recruiter is looking. Any strategy premised on displacement is wrong. The flip comes when screening moves from recruiters skimming profiles to agents that verify at zero marginal cost — and that should be stated as a bet, not a certainty.*

**Q: Google built its Cloud Credential Wallet on Credly rather than building credential infrastructure. What is the most persuasive reading?**

-   Credential issuance is a pure commodity and building it in-house is always wasteful
-   Google's credentials do not carry deep assessment provenance, so a generic badge platform suffices — whereas Coursera's flagship credentials do, and a generic platform cannot represent that **(correct)**
-   Google plans to acquire Credly
-   Google lacked the engineering capacity to build it

*Explanation: Both readings deserve holding, but the second is stronger. The strategic tell to watch is not badge issuance — it is the Labor Market Insights layer attaching labor-market context to credentials, which is the same enrichment play Module 6 recommends. That is the real competition.*

**Q: Which is identified as the most likely way a Coursera Skills Wallet program fails?**

-   Coursera builds a wallet nobody installs
-   A neutral utility like Credly or a government wallet wins the container layer
-   Nobody verifies — beautiful credentials issued into a market that keeps hiring on resumes and LinkedIn profiles **(correct)**
-   Standards fragmentation makes credentials non-portable

*Explanation: Issuance is unilateral; verification is not. Every credentialing initiative that has failed, failed on the demand side. The mitigation is to seed verification inside Coursera for Business — where thousands of enterprises already have a commercial relationship and a reason to verify their own workforce — before asking the open market to care.*

**Q: Why would LinkedIn adding verifiable credential support be good news for Coursera?**

-   It would let Coursera deprecate its own wallet plans
-   It would make LinkedIn a verifier — the scarce resource — with Coursera's signature and assessment provenance as the trust layer under the checkmark **(correct)**
-   It would force LinkedIn to license Coursera's skills taxonomy
-   It would eliminate self-reported claims from the market

*Explanation: Verifiers are scarce; issuers are not. A high-distribution surface that starts checking makes the difference between a rigorous credential and a completion certificate visible exactly where hiring decisions happen. The genuinely threatening scenario is the opposite — LinkedIn becoming an issuer of assessed credentials and capturing both sides.*

-   **LinkedIn's position, honestly** — Wins overwhelmingly on distribution and recruiter workflow — the only dimensions that have mattered so far. Loses on cryptographic verification, assessment provenance, learner ownership, and portability. A self-reported cert on LinkedIn influences more hiring today than every VC ever issued, because it is where the recruiter looks.
-   **Google's dual role** — Google Career Certificates are delivered ON Coursera, making Google a top content partner and a credential brand at once. Google Cloud Credential Wallet is powered by Credly with Labor Market Insights. Google Wallet is the eventual Android credential container. Watch the enrichment layer, not the badge issuance.
-   **Coursera's four differentiating assets** — (1) Assessment depth — proctored exams, auto-graded assignments, identity verification; only expressible by an issuer that has the data. (2) Partner governance with accreditation standing. (3) An existing skills taxonomy to emit as alignment. (4) Two-sided reach — millions of learners and thousands of enterprises in one customer base.
-   **The three failure modes** — (1) Nobody verifies — most likely; mitigate by seeding verification inside C4B first. (2) A neutral utility wins the container layer; mitigate by not fighting for the container and being present in all of them. (3) Coursera builds a wallet nobody installs; mitigate with a custodial web wallet and a deferred native app.
-   **The positioning statement** — Not 'Coursera has a credential wallet' — a feature claim competing on Coursera's weakest layer. Instead: 'A Coursera credential is the most trustworthy machine-readable claim about a person's skills that exists — because it carries the assessment behind it, and any system can check it in a millisecond without asking us.'
-   **Canvas Credentials — the LMS flank** — Instructure issues OB 3.0 from inside the dominant higher-ed LMS, so credentials originate where coursework already lives with no institutional integration needed. If universities issue natively from Canvas, the Coursera credential becomes one input among many and the CLR aggregation layer moves to the LMS. Respond by being trivially ingestible and competing on provenance.

## Module 10: Coursera Implementation Roadmap
...
> **Warning:** How to read the estimates
>
...
| Phase | Window | Eng (peak) | Eng-months | Value created on its own |
| --- | --- | --- | --- | --- |
| 1 — Issuer | 0–3 | 6–8 | ~21 | High — standalone credible |
| 2 — Wallet + delivery | 3–9 | 8–10 | ~54 | High — learners can actually use them |
| 3 — CLR + enterprise | 9–15 | 10–12 | ~66 | Medium-high — the enterprise wedge |
| 4 — Verification API | 12–18 | 6–8 | ~42 | Medium — option value on the agent wave |
| 5 — Ecosystem | 15–24 | 6–8 | ~63 | Medium-high — where demand is realized |
...
**Q: What is the roadmap's biggest departure from the conventional plan, and why?**

-   Skipping CLR 2.0 entirely in favor of individual badges
-   Not leading with a native mobile wallet — ship a custodial web wallet plus OID4VCI export instead, and make the native-app call a data-driven Phase 4 decision **(correct)**
-   Building the verification API before the issuance pipeline
-   Outsourcing issuance to Credly in Phase 1

*Explanation: A native wallet is a permanent product line — 3–4 engineers steady-state, two app stores, OS churn, a protocol treadmill — competing against Apple, Google, and EUDI national wallets that all 27 EU states must ship by December 2026. A custodial web wallet plus standards-compliant export takes weeks instead of quarters and converts the native-app bet into an instrumented decision.*

**Q: Which organizational addition is most commonly omitted from plans like this?**

-   A dedicated mobile engineering team
-   A Credential Operations function — schema governance, tier assignment, revocation decisions, issuance-failure triage — roughly 3–5 non-engineers, closest to a university registrar **(correct)**
-   A blockchain infrastructure team
-   A dedicated standards-body liaison

*Explanation: Plans routinely fund the engineering and forget the registrar-equivalent function that governs schemas, assigns tiers, decides revocations, and triages issuance failures. Related and equally load-bearing: the assessment tier taxonomy must sit under academic integrity, not marketing, or it will be worthless within two quarters of internal lobbying.*

**Q: If Coursera could fund only one piece of work, which should it be?**

-   The wallet app, since learners need somewhere to store credentials
-   The verification API, since verifiers are the scarce resource
-   The alignment and assessment-tier fields inside Phase 1 issuance **(correct)**
-   1EdTech certification, since it unblocks procurement

*Explanation: It is a small data-and-schema project that produces machine-comparable, honestly graded credentials — the one asset competitors cannot copy by buying a vendor. It is also the only irreversible one: you cannot retroactively add fields to a signed document without reissuing, so every month of issuance without them creates credentials that are permanently impoverished.*

**Q: What is the stated kill criterion after Phase 2?**

-   If no enterprise will pay for the workforce skills record
-   If fewer than 5% of eligible learners claim a credential into any wallet within 90 days, the learner demand hypothesis is wrong — stop rather than funding Phase 3 to fix demand with features **(correct)**
-   If paid enrichment verifications fall below a few thousand per day
-   If a neutral utility becomes the default container

*Explanation: Each phase has its own falsification test. Phase 2's tests learner demand; the enterprise-payment test belongs to Phase 3 and the verification-volume test to Phase 4. The discipline being enforced is not adding features to fix a demand problem — a program that cannot be falsified runs forever on narrative.*

-   **The five phases** — P1 (0–3mo) Become an excellent issuer. P2 (3–9mo) Lightweight custodial web wallet + OID4VCI/VP delivery. P3 (9–15mo) CLR 2.0 + enterprise workforce record. P4 (12–18mo) Verification API + enrichment + x402. P5 (15–24mo) ATS distribution and ecosystem. Roughly 245 eng-months total, ~20 engineer-years.
-   **Phase 1 exit criteria** — A third party verifies a Coursera credential using only published standards and a library they did not get from Coursera. That single test proves did:web publication, signing, schema conformance, status lists, and openness all work.
-   **Why Phase 3 is where the money is** — The enterprise workforce skills record — CLRs an enterprise holds across its own workforce, exportable to HRIS, re-verifiable without a Coursera dashboard — is a new budget line (compliance, workforce planning, M&A diligence) rather than a feature of L&D spend. And it outlives the subscription.
-   **The four organizational changes** — New: Credential Operations (3–5 non-engineers, registrar-equivalent). Existing: Security owns key custody with dual control and a compromise runbook before Phase 1 ships. Legal owns revocation policy and GDPR/FERPA posture. Academic integrity owns the T1–T4 taxonomy and defends it from internal upgrade pressure.
-   **The four kill criteria** — P2: <5% of eligible learners claim within 90 days → learner demand hypothesis wrong. P3: no enterprise pays for the workforce record → the strongest commercial hypothesis failed, disqualifying. P4: paid enrichment below a few thousand/day → agent thesis hasn't arrived; keep the free tier, shelve metering. Anytime: a neutral container wins and can't represent your differentiated fields → revisit build-vs-partner now.
-   **The real deadline** — You cannot retroactively add fields to a signed document without reissuing it. Every month of issuance without alignment and assessment-tier fields produces credentials that are permanently impoverished. That makes schema completeness urgent in a way the wallet and API never are.

## Module 11: Technical Deep Dive
...
| --- | --- | --- |
| Shape | Credential JSON with an embedded proof | Credential as a JWT payload; signature in the JWS |
| Canonicalization | Required (RDFC-1.0) — expensive | None — sign the bytes |
| Throughput | Canonicalization-bound | Signature-bound (much faster) |
| Selective disclosure | ecdsa-sd-2023, bbs-2023 | SD-JWT (RFC 9901) |
| Unlinkability | Possible with BBS+ | No — signature is constant |
| Multiple signatures | Natural — several proofs on one document | Awkward |
| Ecosystem pull | 1EdTech, DCC, education tooling | EUDI, mDL, OID4VC, mobile wallets |
| Debuggability | Readable JSON | Base64 blob until decoded |
...
| Cryptosuite / format | Curve | Selective disclosure | Unlinkable | Use for |
| --- | --- | --- | --- | --- |
| eddsa-rdfc-2022 | Ed25519 | No | No | Default. Fast, small, widely supported |
| ecdsa-rdfc-2019 | P-256 / P-384 | No | No | FIPS / HSM environments requiring NIST curves |
| ecdsa-sd-2023 | P-256 | Yes | No | Selective disclosure on NIST curves |
| bbs-2023 | BLS12-381 | Yes | Yes | Privacy-sensitive credentials; still maturing |
| SD-JWT (RFC 9901) | Any JOSE alg | Yes | No | Mobile wallets, EUDI, OID4VC flows |
...
> **Warning:** Status list operational rules
>
...
| Field | Meaning | Guidance for education credentials |
| --- | --- | --- |
| validFrom | When the credential becomes valid | Issuance timestamp. On backfill this is today, not the original completion date. |
| validUntil | When it stops being valid | Usually omit. A completed course does not un-complete. |
| awardedDate (on the subject) | When the achievement occurred | The real completion date. This is what a human cares about. |
| refreshService | Where to obtain a fresh credential | Useful for renewable certifications and for credentials signed with a retired key. |
...
**Q: A verifier receives a credential as a JWT with typ 'vc+jwt' and finds no 'proof' member. What is the correct interpretation?**

-   The credential is unsigned and must be rejected
-   The JWS signature is the proof — this is VC-JOSE-COSE securing, and kid points to the verification method in the issuer's DID document **(correct)**
-   The proof was stripped in transit, indicating tampering
-   Only Data Integrity proofs are valid under VC 2.0

*Explanation: VC 2.0 defines two securing mechanisms. Data Integrity embeds a proof member; VC-JOSE-COSE wraps the credential in a JWT where the JWS signature is the proof. A verifier that only looks for a proof member will reject perfectly valid credentials — a common and confusing interop failure.*

**Q: Why does SD-JWT provide selective disclosure but not unlinkability?**

-   The salts are reused across presentations
-   The JWT signature is identical in every presentation, so two verifiers comparing notes can tell they saw the same credential even if they saw different fields **(correct)**
-   Disclosures contain the holder's DID
-   SD-JWT reveals the total number of hidden claims

*Explanation: The issuer signs hashes of salted claims; the holder reveals a subset by sending only the matching disclosures. That hides values effectively, but the signature over the payload never changes. BBS+ solves this by deriving a fresh zero-knowledge proof each time, which is why it is the only unlinkable option — and why it is still maturing.*

**Q: Which status list operational rule protects the herd-privacy property?**

-   Serving the list with a long cache TTL
-   Not logging requests, and keeping lists large and well-mixed rather than one per course **(correct)**
-   Signing the status list credential
-   Pre-allocating index ranges per worker

*Explanation: Herd privacy means the issuer learns someone checked list #3, not which of ~131,000 credentials. Access logs sell that back, and a per-course list shrinks the anonymity set until the fetch itself reveals which course is being checked. Signing the list and pre-allocating indexes are correct but address integrity and throughput, not privacy.*

**Q: Why should most Coursera credentials omit validUntil?**

-   The VC 2.0 specification discourages expiration dates
-   A completed course is a historical fact that does not decay, and an expired credential reads as invalid rather than old — silently failing millions of legitimate credentials with no alerting event **(correct)**
-   Expiration is incompatible with bitstring status lists
-   Verifiers ignore validUntil in practice

*Explanation: Setting validUntil 'to be safe' creates a rolling wave of legitimate credentials failing verification with nothing to alert on. Expire only what genuinely expires — time-boxed certifications, version-tied credentials, regulatory renewal cycles. For everything else expose awardedDate and let recency be a signal the verifier weighs rather than a cliff you impose.*

**Q: What makes JSON-LD canonicalization expensive, and what is the mandatory mitigation?**

-   Signature computation over large graphs; mitigate by using smaller keys
-   Expanding terms against @context and running RDFC-1.0 with deterministic blank-node labelling; pin contexts locally so expansion never hits the network **(correct)**
-   Base64 encoding overhead; mitigate with compression
-   Schema validation against remote JSON Schema files; mitigate by caching schemas

*Explanation: Data Integrity signs a canonical graph form, not the JSON bytes, so identical semantics yield identical signatures. Pinning contexts locally is mandatory for performance and for security — a remote context is effectively remote code execution over your semantics. For bulk backfill, JWT securing skips canonicalization entirely and turns a multi-week job into a multi-day one.*

-   **The two securing mechanisms** — Data Integrity (embedded proof, JSON-LD canonicalization required, supports BBS+ unlinkability, favored by 1EdTech/education tooling) vs VC-JOSE-COSE (JWT/SD-JWT, no canonicalization, much faster, favored by EUDI/mDL/OID4VC/mobile wallets). Support both; keep the signing service format-agnostic so the dual path lives in one component.
-   **Cryptosuite defaults** — eddsa-rdfc-2022 (Ed25519) is the right default — 64-byte signatures, fast, no curve footguns, universal support. ecdsa-rdfc-2019 (P-256) for FIPS/HSM. ecdsa-sd-2023 for selective disclosure on NIST curves. bbs-2023 (BLS12-381) for unlinkability, still maturing. SD-JWT for mobile/EUDI/OID4VC.
-   **How SD-JWT works in five steps** — Issuer salts and hashes each disclosable claim → puts only hashes in the signed payload under \_sd → sends JWT plus all disclosures (salt+name+value) → holder presents JWT plus only the disclosures being revealed → verifier re-hashes each disclosure and confirms it appears in \_sd. Hidden claims are unknowable; the signature never changes, hence linkability.
-   **Bitstring Status List sizing and semantics** — GZIP-compressed base64url bitstring, minimum 131,072 entries (16 KB raw, ~2 KB compressed since revocations are rare). Each credential carries its index. statusPurpose distinguishes permanent revocation from reversible suspension. The list is itself a signed VC, so verifiers can trust the revocation data.
-   **Five status list operational rules** — CDN with short (minutes) TTL — long delays revocation, none serves the world's fetch volume. Don't log requests (that's selling back herd privacy). Keep lists large and well-mixed, never one per course. Pre-allocate index ranges per worker. Never reuse an index — it silently transfers revocation state.
-   **The four lifecycle date fields** — validFrom = when the credential became valid (on backfill: today). validUntil = usually omit; a completed course doesn't un-complete. awardedDate on the subject = the real completion date, what humans care about. refreshService = where to get a fresh credential, useful for renewable certs and retired-key reissuance.
-   **Retired key = warning, not failure** — A credential signed with a key that has since been retired is still valid — that's exactly why retired verification methods are never deleted from the DID document. A verifier should surface it as a warning. Collapsing warnings into failures makes verifiers reject legitimate credentials en masse.

## Module 12: Privacy & Compliance
...
| GDPR principle | Today (issuer-hosted) | With verifiable credentials |
| --- | --- | --- |
| Data minimisation (Art. 5) | Verification page reveals whatever the page shows | Better — selective disclosure reveals only requested fields |
| Purpose limitation | Issuer sees every verification and could repurpose it | Better — the issuer is not in the transaction |
| Data portability (Art. 20) | Export produces a PDF | Much better — machine-readable, interoperable, a live artifact |
| Right of access (Art. 15) | Subject access request to the issuer | Better — the learner already holds the data |
| Right to erasure (Art. 17) | Delete the database row | Harder — you cannot delete a copy someone else holds |
| Correlation risk | Bounded — the issuer is the only observer | New risk — a static DID is a cross-verifier identifier |
...
> **Warning:** The FERPA trap in the enterprise product
>
...
| --- | --- | --- |
| Pairwise DIDs | A distinct subject DID per verifier relationship | Credentials are bound to one subject DID, so this needs per-relationship issuance or re-binding — substantial |
| BBS+ derived proofs | Fresh unlinkable proof per presentation | Cryptosuite and library maturity; an L1 decision made at issuance |
| Batch issuance of single-use credentials | Issue N copies with distinct identifiers; use each once | Storage and issuance overhead; refresh logistics |
| Accept it, disclose it | Document that presentations are correlatable | Free — and honest, which counts for more than it seems |
...
-   **Course later found to be substandard** — do not revoke. The learner did the work in good faith. Deprecate the achievement definition instead so no new credentials are issued.
-   **Partner relationship ends** — do not revoke. The achievement happened. Nothing about it became false.
...
**Q: How is the GDPR right to erasure satisfied for a credential already in a learner's wallet?**

-   The issuer sends a deletion command to the wallet over DIDComm
-   Delete Coursera's copy and issuance record, revoke via the status list, recognize the learner's own copy as theirs, and treat verifier copies as the verifier's controller obligation **(correct)**
-   Erasure does not apply to verifiable credentials
-   The credential's validUntil is set to the current time

*Explanation: Erasure rights run against controllers, not against a data subject's own records — a learner holding a document about themselves is not Coursera processing their data. Revocation makes the credential inert wherever it exists. This is materially better than a PDF emailed to fifty employers, which is equally unreachable and has no revocation mechanism at all.*

**Q: What residual GDPR problem does the status list create?**

-   It publishes learner names in a public bitstring
-   Keeping a revoked credential revoked requires retaining an index-to-learner mapping indefinitely — ongoing processing that persists after an erasure request **(correct)**
-   Status lists cannot be hosted in the EU
-   Verifiers can enumerate all revoked credentials

*Explanation: The bitstring itself contains no learner data, but the mapping needed to keep the right bit set is personal data retained after erasure. Mitigations: a separate minimal store holding only index and hash under its own retention basis, or accepting un-revocation on erasure and documenting it. Decide deliberately in Phase 1, not in a Phase 3 audit.*

**Q: Why must the Coursera for Business workforce skills record be built on learner-consented presentations rather than issuer-side data export?**

-   Data export is technically harder at enterprise scale
-   An employer receiving a CLR about a sponsored learner in a FERPA-covered program is third-party disclosure needing a consent basis independent of employment **(correct)**
-   Exported data cannot be cryptographically signed
-   GDPR prohibits bulk data export to enterprises

*Explanation: The two paths produce a superficially identical dashboard and are legally very different objects. A VC presentation is itself a consent event — the learner chose to present, to a named verifier, at a specific time — which maps to FERPA's model cleanly. Retrofitting consent into a shipped enterprise feature is far worse than designing for it.*

**Q: What is the recommended position on DID correlation for ordinary course credentials?**

-   Implement pairwise DIDs for every verifier relationship from day one
-   Accept correlation and disclose it plainly — the learner is already presenting under their real name to a named employer — but use BBS+ from day one for any sensitive-category credentials **(correct)**
-   Issue batches of single-use credentials for every learner
-   Use BBS+ universally regardless of credential type

*Explanation: For employment credentials the learner has already told the verifier who they are, so a stable DID adds little. Correlation matters intensely for anonymous skill marketplaces and credentials touching protected categories — disability accommodations, recovery programs, immigration or veteran status. Since the securing decision is effectively irreversible at scale, sensitive classes need BBS+ from the start.*

**Q: Which scenario should NOT trigger revocation?**

-   An academic integrity violation discovered after issuance
-   An issuance error assigning the wrong course or tier
-   A partner relationship ending, or a course later judged substandard **(correct)**
-   A valid legal order

*Explanation: Revocation is for statements that were never true or are no longer true — not a business lever. The learner did the work in good faith and nothing about the achievement became false. Deprecate the achievement definition so no new credentials issue. Revoking for business reasons changes the signature's meaning from 'this is true' to 'Coursera currently endorses this,' which is worth far less.*

-   **GDPR scorecard for VCs** — Better: data minimisation (selective disclosure), purpose limitation (issuer not in the transaction), portability (machine-readable), access (learner already holds it). Harder: erasure (can't delete someone else's copy). New: correlation risk from a static DID acting as an uncleanable cross-verifier cookie.
-   **The erasure paradox, four-part answer** — (1) Delete Coursera's copy and issuance record. (2) Revoke via status list so future verification fails. (3) The learner's own copy is theirs — erasure runs against controllers, not the data subject. (4) Verifier copies are the verifier's own controller obligation, same as a PDF today.
-   **Why FERPA fits VCs unusually well** — Disclosure becomes learner-initiated by construction — a presentation IS a consent event, naming the verifier and the time. Selective disclosure is record-level data minimisation. The audit trail lives in the learner's wallet where they can see it, rather than in an institutional log they cannot.
-   **The FERPA trap in the enterprise product** — An employer receiving a CLR about a sponsored learner in a FERPA-covered program is third-party disclosure requiring a consent basis independent of employment. Build the workforce record on learner-consented presentations, not issuer-side export — identical dashboard, very different legal object.
-   **The accessibility requirement** — Wallet-first assumes a modern smartphone with hardware-backed keys and app-store access — excluding exactly the learners for whom portable credentials matter most. The web wallet must work on low-end devices and slow connections, a downloadable file must work with no wallet, and the hosted verification page must persist indefinitely. Not a degraded tier.
-   **The revocation principle** — Revocation is for statements that were never true or are no longer true — never a business lever. Revoke for integrity violations, issuance errors, learner request, legal order. Do NOT revoke for a soured partnership or a course later judged substandard. Publish the policy in Phase 1 as an ecosystem commitment.
-   **The hidden compliance cost of a custodial wallet** — Operating a custodial wallet makes Coursera a processor (arguably controller) for credentials issued by OTHER parties that learners store there — obligations over data Coursera didn't create and can't validate. Export-first architecture keeps that surface small: a compliance argument on top of the cost and strategy ones.

## Module ★: Resources & Primary Sources

### Specifications

-   [W3C Verifiable Credentials Data Model v2.0](https://www.w3.org/TR/vc-data-model-2.0/) — Recommendation, 15 May 2025
-   [W3C Verifiable Credentials Overview](https://www.w3.org/TR/vc-overview/) — the best single orientation to the seven-spec family
-   [W3C press release — VC 2.0 published as a standard](https://www.w3.org/press-releases/2025/verifiable-credentials-2-0/)
-   [1EdTech Open Badges 3.0 specification](https://www.imsglobal.org/spec/ob/v3p0) and the [implementation guide](https://www.imsglobal.org/spec/ob/v3p0/impl)
-   [1EdTech Open Badges overview](https://www.1edtech.org/standards/open-badges) · [CLR & Open Badges FAQ](https://www.1edtech.org/clr/faq)
-   [1EdTech — Connecting the Dots: making sense of digital credential standards](https://www.1edtech.org/blog/connecting-the-dots-making-sense-of-digital-credential-standards)
-   [Open Badges 3.0 certification guide](https://www.imsglobal.org/spec/ob/v3p0/cert)

### Wallets and reference implementations

-   [Learner Credential Wallet](https://github.com/openwallet-foundation-labs/learner-credential-wallet) — OpenWallet Foundation Labs (iOS + Android, open source)
-   [MIT Open Learning — DCC transfers LCW stewardship to the OpenWallet Foundation](https://openlearning.mit.edu/news/dcc-transfers-stewardship-learner-credential-wallet-openwallet-foundation)
-   [MIT Learn — What is the DCC and Open Badges 3.0?](https://support.learn.mit.edu/hc/en-us/articles/45262917914267-What-is-the-Digital-Credentials-Consortium-DCC-and-Open-Badges-3-0)
-   [ASU Pocket — a digital wallet for real-time achievements](https://pocket.asu.edu/asu-pocket-digital-wallet-capture-learners-real-time-achievements)
-   [ASU — how the Trusted Learner Network and ASU Pocket put privacy first](https://tln.asu.edu/blog/how-asus-trusted-learner-network-and-asu-pocket-put-privacy-first)
-   [ASU Pocket open beta announcement](https://tech.asu.edu/asu-pocket-launches-open-beta-asu-community)

### Market research and primary data

-   [Coursera Micro-Credentials Impact Report 2026](https://blog.coursera.org/coursera-micro-credentials-impact-report-2026/) — the 94% / 92% / 86% figures
-   [Credential Engine — Counting Credentials 2025](https://credentialengine.org/all-resources/2025-counting-credentials/) (December 2025)
-   [Counting Credentials 2025 full report (PDF)](https://credentialengine.org/wp-content/uploads/2025/12/Counting-Credentials-2025-Report.pdf)
-   [WCET — 1.85 million credentials and counting](https://wcet.wiche.edu/frontiers/2026/04/09/millions-of-credentials-digital-marketplace-is-here/)

### Ecosystem and policy

-   [Velocity Network Foundation](https://www.velocitynetwork.foundation/) — the Internet of Careers
-   [Credential Engine's profile of Velocity Network](https://credentialengine.org/toolkit/velocity-network/)
-   [US DOE — $15M Connecting Talent to Opportunity Challenge launch](https://www.ed.gov/about/news/press-release/us-department-of-education-launches-15-million-challenge-create-next-generation-of-talent-marketplaces)
-   [US DOE — Challenge semifinalists announced](https://www.ed.gov/about/news/press-release/us-department-of-education-announces-connecting-talent-opportunity-challenge-semifinalists)
-   [European Digital Credentials for Learning (Europass)](https://europass.europa.eu/en/european-digital-credentials-learning)
-   [Europass — how to receive and use a European Digital Credential](https://europass.europa.eu/en/news/how-receive-and-use-european-digital-credential)
-   [EUDI Wallet Hub](https://www.eudi-wallet.eu/) — eIDAS 2, use cases, standards
-   [Credly — support for Open Badges 3.0](https://learn.credly.com/blog/credly-supports-open-badge-3.0) · [1EdTech certification listing](https://site.imsglobal.org/certifications/credly-by-pearson/credly)
-   [Google Cloud Credential Wallet announcement](https://cloud.google.com/blog/topics/training-certifications/introducing-the-google-cloud-credential-wallet)

### Protocols and cryptography

-   [OpenID for Verifiable Credential Issuance — developer overview](https://www.authlete.com/developers/oid4vci/)
-   [OpenID4VC explained — OID4VCI, OID4VP, SIOPv2](https://docs.igrant.io/concepts/openID4vc/)
-   [Digital Credentials API in browsers (2026 status)](https://www.corbado.com/blog/digital-credentials-api)
-   [OpenID VC interoperability testing results](https://www.biometricupdate.com/202507/openid-vc-spec-shows-interoperability-between-issuers-digital-wallets)

### Related courses in this collection

-   [Cloudflare Wallets](cloudflare-wallets-course.html) — the payments half of Module 7: x402 on the wire, Account vs Virtual Wallets, the Monetization Gateway
-   [Cloudflare for Developers](cloudflare-platform-course.html) — Workers, KV, R2, and the edge platform the verification API would run on
-   [AI-Native Learning](ai-native-learning-course.html) — the assessment and pedagogy side of what a credential should attest to
-   [Instructure](instructure-company-course.html) — Canvas Credentials and the LMS flank from Module 9
...
**Skills Wallet Architecture** — built for Mustafa Furniturewala, VP Engineering, Coursera · August 2026

Standards status current as of August 2026. Engineering estimates are directional and outside-in. [← All courses](index.html)